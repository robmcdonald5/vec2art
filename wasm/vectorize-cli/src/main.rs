...
use vectorize_core::{
    ...
    LogoConfig, RegionsConfig, SvgConfig,
    VectorizeError,
};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Logo { input, output, config, threshold, adaptive, simplify } => {
            vectorize_logo_command(input, output, config, threshold, adaptive, simplify)
        }
        Commands::Regions { input, output, config, colors, lab_color, simplify } => {
            vectorize_regions_command(input, output, config, colors, lab_color, simplify)
        }
        Commands::Benchmark { input, algorithm, iterations, output } => {
            benchmark_command(input, algorithm, iterations, output)
        }
        Commands::Batch { input, output, algorithm, config, pattern } => {
...
fn vectorize_logo_command(
    ...
    let svg_content = vectorize_logo_rgba(&img, &config)
...
fn vectorize_regions_command(
    ...
    let svg_content = vectorize_regions_rgba(&img, &config)
...
fn batch_command(
    ...
    log::info!("Batch processing from {} to {}", input.display(), output.display());
...
impl BenchmarkResult {
    fn print_summary(&self) {
        ...
        println!("\n{} Algorithm Benchmark Results:", self.algorithm.to_uppercase());
