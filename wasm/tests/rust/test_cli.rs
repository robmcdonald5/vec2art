...
pub fn run_basic_validation() -> Result<()> {
    ...
    if output.status.success() {
        println!("  ✅ Basic dot mapping PASSED in {:.0}ms", basic_time.as_millis());
    } else {
    ...
    if output.status.success() {
        println!("  ✅ Color preservation PASSED in {:.0}ms", color_time.as_millis());
    } else {
    ...
    let output = Command::new("cargo")
        .args(&[
            ...
            "--backend", "dots",
            "--dot-density", "1.5" // Invalid: too high
        ])
        .output()
    ...
    if output.status.success() {
        ...
        if time_ms <= 1500 {
            println!("  ✅ Performance benchmark PASSED in {:.0}ms (target: <1500ms)", time_ms);
        } else {
            println!("  ⚠️ Performance benchmark SLOW in {:.0}ms (target: <1500ms)", time_ms);
        }
    } else {
