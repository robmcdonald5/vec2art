...
fn test_edge_detection_with_sample_image() {
    ...
    // Create a simple vertical edge (black left, white right) to test edge detection
    let image = image::ImageBuffer::from_fn(100, 100, |x, _y| {
        if x < 50 {
            image::Rgb([0u8, 0u8, 0u8]) // Black left half
        } else {
            image::Rgb([255u8, 255u8, 255u8]) // White right half
        }
    });
    ...
    // Get default parameters but with lower thresholds for this simple black/white test image
    // Note: Default thresholds (50.0, 150.0) are too high for simple synthetic test images
    // but work well for real photos with noise and complex gradients
    let params_json = r#"{
        "algorithm": "EdgeDetector",
        "method": "Canny",
        "threshold_low": 10.0,
        "threshold_high": 30.0,
        "gaussian_sigma": 1.0,
...
#[test]
#[ignore = "GeometricFitter disabled pending major refactor"]
fn test_geometric_fitter_with_sample_image() {
    ...
    // Use optimized parameters for memory-efficient testing
    let params_json = r#"{
...
fn test_all_algorithms_with_real_image() {
    ...
    if !std::path::Path::new(test_image_path).exists() {
        println!(
            "Skipping real image test - place a test image at {}",
            test_image_path
        );
        return;
    }
    ...
    // Test geometric fitter - NOW ENABLED with memory optimizations
    let params = get_default_params_native("geometric_fitter").unwrap();
    let svg = convert_native(&image_bytes, &params).unwrap();
