...
use vectorize_core::algorithms::dots::{DotConfig, generate_dots};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Foreground ratio: {:.2}%", (foreground_pixels as f32 / total_pixels as f32) * 100.0);
    ...
    for y in (0..rgba_img.height()).step_by(20) {
        for x in (0..rgba_img.width()).step_by(20) {
            ...
            println!("  ({}, {}): pixel={:?}, is_background={}", x, y, pixel, is_bg);
        }
    }
    ...
    if foreground_pixels > 0 {
        ...
        let dots_low = generate_dots(&rgba_img, &gradient_analysis, &background_mask, &dot_config_low);
        println!("Dot generation with threshold 0.01: {} dots", dots_low.len());
    }
