...
impl std::fmt::Display for ValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ValidationError::TypeMismatch { parameter, expected, actual } => {
                write!(f, "Parameter '{}' type mismatch: expected {}, got {}", parameter, expected, actual)
            }
            ValidationError::ValueOutOfRange { parameter, value, min, max } => {
                ...
                write!(f, "Parameter '{}' value '{}' is outside {}", parameter, value, range)
            }
            ValidationError::InvalidEnumVariant { parameter, value, valid_variants } => {
                write!(f, "Parameter '{}' invalid value '{}', must be one of: {}", 
                    parameter, value, valid_variants.join(", "))
            }
            ValidationError::MissingRequiredParameter { parameter, required_by } => {
                write!(f, "Parameter '{}' is required when '{}' is set", parameter, required_by)
            }
            ValidationError::ConflictingParameters { parameter1, parameter2 } => {
                write!(f, "Parameters '{}' and '{}' cannot be used together", parameter1, parameter2)
            }
            ValidationError::ParameterNotApplicable { parameter, backend, applicable_backends } => {
                write!(f, "Parameter '{}' does not apply to {:?} backend, applicable backends: {:?}", 
                    parameter, backend, applicable_backends)
            }
            ValidationError::LogicalInconsistency { parameter1, parameter2, description } => {
                write!(f, "Logical inconsistency between '{}' and '{}': {}", parameter1, parameter2, description)
            }
            ValidationError::PerformanceConflict { parameters, description, severity } => {
                write!(f, "[{:?}] Performance conflict with parameters [{}]: {}", 
                    severity, parameters.join(", "), description)
            }
        }
    }
}
...
impl std::fmt::Display for ValidationWarning {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ValidationWarning::ParameterIgnored { parameter, backend, applicable_backends } => {
                write!(f, "Parameter '{}' ignored for {:?} backend (applicable: {:?})", 
                    parameter, backend, applicable_backends)
            }
            ValidationWarning::DeprecatedParameter { old_name, new_name } => {
                write!(f, "Parameter '{}' is deprecated, use '{}' instead", old_name, new_name)
            }
            ValidationWarning::RedundantParameter { parameter, default_value } => {
                write!(f, "Parameter '{}' set to default value ({}), can be omitted", 
                    parameter, default_value)
            }
            ValidationWarning::PerformanceWarning { parameters, description, estimated_impact } => {
                write!(f, "Performance warning for parameters [{}]: {} (estimated impact: {:.1}%)", 
                    parameters.join(", "), description, estimated_impact)
            }
            ValidationWarning::ConflictResolved { conflicting_parameters, resolution } => {
                write!(f, "Conflict resolved for parameters [{}]: {}", 
                    conflicting_parameters.join(", "), resolution)
            }
        }
    }
}
...
impl ParameterValidator {
    ...
    pub fn validate_config(&mut self, config: &RawConfig, backend: TraceBackend) -> ValidationResult {
        ...
        for (param_name, param_value) in config {
            ...
            self.validate_single_parameter(
                param_name, 
                param_value, 
                backend, 
                config,
    ...
    fn validate_single_parameter(
        ...
        let param_def = match PARAMETER_REGISTRY.get(param_name) {
            Some(def) => def,
            None => {
                result.errors.push(ValidationError::UnknownParameter(param_name.to_string()));
                return;
            }
        };
        ...
        for (param_name, param_def) in PARAMETER_REGISTRY.iter() {
            ...
            for required_param in &param_def.constraints.requires {
                if !config.contains_key(required_param) {
                    result.errors.push(ValidationError::MissingRequiredParameter {
    ...
    fn validate_parameter_conflicts(
        &self,
        config: &RawConfig,
        result: &mut ValidationResult
    ) {
    ...
    fn check_logical_inconsistencies(&self, config: &RawConfig, result: &mut ValidationResult) {
        ...
        if let (Some(ParameterValue::Boolean(false)), Some(_)) = 
            (config.get("noise_filtering"), config.get("noise_filter_spatial_sigma")) {
            result.warnings.push(ValidationWarning::ConflictResolved {
                conflicting_parameters: vec!["noise_filtering".to_string(), "noise_filter_spatial_sigma".to_string()],
                resolution: "Spatial sigma parameter will be ignored since noise filtering is disabled".to_string(),
            });
        }
        
        // Threshold range validation
        if let (Some(ParameterValue::Float(low)), Some(ParameterValue::Float(high))) = 
            (config.get("canny_low_threshold"), config.get("canny_high_threshold")) {
            if low >= high {
                result.errors.push(ValidationError::LogicalInconsistency {
        ...
        if let (Some(ParameterValue::Float(detail)), Some(ParameterValue::Integer(passes))) = 
            ...
            if *detail > 0.8 && *passes > 3 {
                result.warnings.push(ValidationWarning::PerformanceWarning {
                    ...
                    description: "High detail with multiple passes will significantly slow processing".to_string(),
        ...
        if let Some(ParameterValue::Integer(superpixels)) = config.get("num_superpixels") {
            if *superpixels > 500 {
                result.warnings.push(ValidationWarning::PerformanceWarning {
                    ...
                    description: "Large number of superpixels may cause memory issues and slow processing".to_string(),
        ...
        if let Some(ParameterValue::Float(high_threshold)) = config.get("canny_high_threshold") {
            if *high_threshold > 0.8 {
                result.warnings.push(ValidationWarning::PerformanceWarning {
                    ...
                    description: "Very high Canny threshold may result in too few edges detected".to_string(),
        ...
        if let (Some(ParameterValue::Float(density)), Some(ParameterValue::Float(min_radius))) = 
            ...
            if *density > 0.8 && *min_radius < 1.0 {
                result.warnings.push(ValidationWarning::PerformanceWarning {
                    ...
                    description: "High dot density with small minimum radius creates many small dots".to_string(),
    ...
    fn check_complex_conflicts(&self, config: &RawConfig, result: &mut ValidationResult) {
        ...
        if let (Some(ParameterValue::Boolean(true)), Some(ParameterValue::Boolean(false))) = 
            (config.get("enable_multipass"), config.get("noise_filtering")) {
            result.warnings.push(ValidationWarning::ConflictResolved {
                conflicting_parameters: vec!["enable_multipass".to_string(), "noise_filtering".to_string()],
                resolution: "Multi-pass processing without noise filtering may produce inconsistent results".to_string(),
            });
        }
        
        // Superpixel pattern conflicts
        if let (Some(ParameterValue::String(pattern)), Some(ParameterValue::Integer(count))) = 
            (config.get("superpixel_initialization_pattern"), config.get("num_superpixels")) {
        ...
        if let (Some(ParameterValue::Boolean(true)), Some(ParameterValue::Integer(passes))) = 
            (config.get("enable_color_quantization"), config.get("pass_count")) {
            if *passes > 2 {
                result.warnings.push(ValidationWarning::PerformanceWarning {
                    parameters: vec!["enable_color_quantization".to_string(), "pass_count".to_string()],
                    description: "Color quantization with multiple passes may over-simplify the output".to_string(),
...
mod tests {
    ...
    fn test_valid_configuration() {
        ...
        config.insert("superpixel_initialization_pattern".to_string(), 
                     ParameterValue::String("poisson".to_string()));
        
        let result = validate_configuration(&config, TraceBackend::Superpixel);
        assert!(result.is_valid(), "Configuration should be valid: {:?}", result.errors);
    }
    ...
    fn test_invalid_enum_value() {
        ...
        let errors: Vec<_> = result.errors.iter()
            .filter_map(|e| match e {
                ValidationError::InvalidEnumVariant { parameter, value, .. } => Some((parameter, value)),
                _ => None,
            })
            .collect();
    ...
    fn test_parameter_dependencies() {
        let mut config = RawConfig::new();
        config.insert("superpixel_boundary_epsilon".to_string(), ParameterValue::Float(1.0));
    ...
    fn test_logical_inconsistency_multipass() {
        let mut config = RawConfig::new();
        config.insert("enable_multipass".to_string(), ParameterValue::Boolean(true));
    ...
    fn test_logical_inconsistency_canny_thresholds() {
        let mut config = RawConfig::new();
        config.insert("canny_low_threshold".to_string(), ParameterValue::Float(0.8));
        config.insert("canny_high_threshold".to_string(), ParameterValue::Float(0.6)); // low > high
    ...
    fn test_performance_warning_high_detail_multipass() {
        ...
        assert!(has_performance_warning, "Should warn about performance impact");
    }
    ...
    fn test_performance_warning_large_superpixel_count() {
        ...
        let result = validate_configuration(&config, TraceBackend::Superpixel);
        assert!(result.has_warnings(), "Should generate performance warning for large superpixel count");
        ...
        assert!(has_superpixel_warning, "Should warn about superpixel performance impact");
    }
    ...
    fn test_performance_warning_high_dot_density() {
        ...
        let result = validate_configuration(&config, TraceBackend::Dots);
        assert!(result.has_warnings(), "Should generate performance warning for high dot density");
        ...
        assert!(has_dot_warning, "Should warn about dot density performance impact");
    }
    ...
    fn test_complex_conflict_poisson_high_count() {
        ...
        let result = validate_configuration(&config, TraceBackend::Superpixel);
        assert!(result.has_warnings(), "Should warn about Poisson pattern with high count");
        ...
        assert!(has_poisson_warning, "Should warn about Poisson pattern performance");
    }
    ...
    fn test_complex_conflict_hexagonal_low_count() {
        ...
        let result = validate_configuration(&config, TraceBackend::Superpixel);
        assert!(result.has_warnings(), "Should warn about hexagonal pattern with low count");
        ...
        assert!(has_hexagonal_warning, "Should warn about hexagonal pattern optimization");
    }
    
    #[test]
    fn test_noise_filtering_dependency_warning() {
        let mut config = RawConfig::new();
        config.insert("noise_filtering".to_string(), ParameterValue::Boolean(false));
        config.insert("noise_filter_spatial_sigma".to_string(), ParameterValue::Float(1.5));
        
        let result = validate_configuration(&config, TraceBackend::Edge);
        assert!(result.has_warnings(), "Should warn about noise filter dependency");
        ...
        assert!(has_dependency_warning, "Should warn about ignored noise filter parameter");
    }
    
    #[test]
    fn test_color_quantization_multipass_warning() {
        let mut config = RawConfig::new();
        config.insert("enable_color_quantization".to_string(), ParameterValue::Boolean(true));
        config.insert("pass_count".to_string(), ParameterValue::Integer(3));
        
        let result = validate_configuration(&config, TraceBackend::Edge);
        assert!(result.has_warnings(), "Should warn about color quantization with multipass");
        ...
        assert!(has_quantization_warning, "Should warn about over-simplification risk");
    }
    
    #[test]
    fn test_high_canny_threshold_warning() {
        let mut config = RawConfig::new();
        config.insert("canny_high_threshold".to_string(), ParameterValue::Float(0.9));
        
        let result = validate_configuration(&config, TraceBackend::Edge);
        assert!(result.has_warnings(), "Should warn about very high Canny threshold");
        
        let has_canny_warning = result.warnings.iter().any(|w| {
            matches!(w, ValidationWarning::PerformanceWarning { parameters, .. } 
                if parameters.contains(&"canny_high_threshold".to_string()))
        });
        assert!(has_canny_warning, "Should warn about potential sparse edge detection");
    }
    ...
    fn test_multipass_without_noise_filtering_warning() {
        ...
        let result = validate_configuration(&config, TraceBackend::Edge);
        assert!(result.has_warnings(), "Should warn about multipass without noise filtering");
        ...
        assert!(has_multipass_warning, "Should warn about inconsistent multipass configuration");
    }
}