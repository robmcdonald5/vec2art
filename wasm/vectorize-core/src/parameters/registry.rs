...
use super::types::{ParameterCategory, ParameterConstraints, ParameterType, ParameterValue};
    ...
    vec![
        ...
        (
            ...
            ParameterDefinition {
                ...
                constraints: ParameterConstraints {
                    requires: vec![
                        "enable_reverse_pass".to_string(),
                        "enable_diagonal_pass".to_string(),
                    ],
                    backend_specific: true,
                    ..Default::default()
                },
...
pub fn validate_registry_completeness() -> Result<(), String> {
    ...
    for (name, def) in PARAMETER_REGISTRY.iter() {
        ...
        if def.name != *name {
            return Err(format!(
                "Parameter name mismatch: registry key '{}' != def.name '{}'",
                name, def.name
            ));
        }
    }
...
mod tests {
    ...
    fn test_backend_parameter_filtering() {
        ...
        assert!(superpixel_params
            .iter()
            .any(|p| p.name == "superpixel_initialization_pattern"));
        assert!(superpixel_params
            .iter()
            .any(|p| p.name == "num_superpixels"));
        ...
        assert!(!superpixel_params
            .iter()
            .any(|p| p.name == "dot_density_threshold"));
    }
    ...
    fn test_parameter_lookup() {
        ...
        assert!(param_def
            .applicable_backends
            .contains(&TraceBackend::Superpixel));
    }
    ...
    fn test_parameter_constraints() {
        ...
        assert!(param_def
            .constraints
            .requires
            .contains(&"superpixel_simplify_boundaries".to_string()));
    }
}
