...
fn generate_main_config_interface(output: &mut String) {
    ...
    let mut params_by_category: BTreeMap<ParameterCategory, Vec<&ParameterDefinition>> =
        BTreeMap::new();
    ...
    for (category, params) in params_by_category {
        ...
        for param in sorted_params {
            ...
            let optional = if param.constraints.backend_specific {
                "?"
            } else {
                ""
            };
...
fn generate_backend_interfaces(output: &mut String) {
    ...
    for backend in backends {
        ...
        output.push_str(&format!(
            "export interface {}BackendConfig extends Pick<VectorizerConfig,\n",
            backend_name
        ));
...
fn generate_parameter_metadata(output: &mut String) {
    ...
    for def in PARAMETER_REGISTRY.values() {
        ...
        match &def.parameter_type {
            super::types::ParameterType::Float {
                min,
                max,
                precision,
            } => {
...
mod tests {
    ...
    fn test_parameter_metadata_completeness() {
        ...
        for param_name in PARAMETER_REGISTRY.keys() {
            assert!(
                typescript_code.contains(&format!("  '{}': {{", param_name)),
                "Parameter {} not found in generated metadata",
                param_name
            );
        }
    }
