...
fn rgba_to_lab_simd(rgba: &Rgba<u8>) -> [f32; 3] {
    ...
    let r_linear = if r <= 0.04045 { r / 12.92 } else { ((r + 0.055) / 1.055).powf(2.4) };
    let g_linear = if g <= 0.04045 { g / 12.92 } else { ((g + 0.055) / 1.055).powf(2.4) };
    let b_linear = if b <= 0.04045 { b / 12.92 } else { ((b + 0.055) / 1.055).powf(2.4) };
    ...
    // Convert XYZ to LAB
    let fx = if x_n > 0.008856 { x_n.powf(1.0/3.0) } else { 7.787 * x_n + 16.0/116.0 };
    let fy = if y_n > 0.008856 { y_n.powf(1.0/3.0) } else { 7.787 * y_n + 16.0/116.0 };
    let fz = if z_n > 0.008856 { z_n.powf(1.0/3.0) } else { 7.787 * z_n + 16.0/116.0 };
            ...
            if centroids.len() >= 4 {
                ...
                for centroid_batch_start in (0..centroids.len()).step_by(4) {
                    ...
                    if centroid_batch_size == 4 {
                        ...
                        #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
                        unsafe {
                            simd_color_distance_lab_x4(&color_batch_same, &centroid_batch, &mut distances);
                        }
                        #[cfg(not(any(target_arch = "x86", target_arch = "x86_64")))]
                        {
                            for j in 0..4 {
                                distances[j] = scalar_color_distance_lab(&color, &centroid_batch[j]);
                            }
                        }
...
pub fn simd_analyze_gradient_strength(colors: &[Rgba<u8>], threshold: f32) -> Vec<f32> {
    ...
    if colors.len() >= 2 {
        ...
        gradient_strengths[last_idx] = scalar_color_distance_lab(&colors[last_idx - 1], &colors[last_idx]);
    }
