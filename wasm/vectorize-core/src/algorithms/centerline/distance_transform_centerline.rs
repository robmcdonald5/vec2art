...
use super::{CenterlineAlgorithm, ThresholdingStrategy, PerformanceProfile, Complexity, MemoryUsage};
use super::thresholding::{BradleyRothThresholding, SauvolaThresholding};
use super::distance_transform::{FelzenszwalbHuttenlocher, DistanceFieldCenterlineExtractor};
use super::preprocessing::MinimalPreprocessing;
use crate::algorithms::{Point, SvgPath};
use crate::error::VectorizeError;
use crate::TraceLowConfig;
use image::{GrayImage, ImageBuffer, Rgba, Luma};
...
impl DistanceTransformCenterlineAlgorithm {
    ...
    pub fn with_high_performance_settings() -> Self {
        ...
        let extractor = DistanceFieldCenterlineExtractor::with_strategy(
            Box::new(SimdDistanceTransform)
        );
...
impl CenterlineAlgorithm for DistanceTransformCenterlineAlgorithm {
    fn extract_centerlines(
        ...
        let phase_start = Instant::now();
        let blur_sigma = if config.detail < 0.3 { 0.8 } else if config.detail > 0.7 { 1.2 } else { 1.0 };
...
fn otsu_threshold_optimized(gray: &GrayImage) -> GrayImage {
    ...
    for threshold in 1..255 {
        ...
        let between_class_variance = (weight_bg * weight_fg * (mean_bg - mean_fg).powi(2)) / total_pixels as f64;
...
fn dp_recursive_simple(points: &[Point], start: usize, end: usize, epsilon: f32, result: &mut Vec<Point>) {
    ...
    for i in (start + 1)..end {
        let point = &points[i];
        let distance = ((point.x - start_point.x) * dy - (point.y - start_point.y) * dx).abs() / line_length;
...
fn bridge_endpoints_smart(polylines: Vec<Vec<Point>>, max_distance: f32, max_angle_deg: f32) -> Vec<Vec<Point>> {
...
fn polylines_to_svg_paths_optimized(
    ...
    let svg_paths: Vec<SvgPath> = polylines.into_iter()
        .map(|polyline| {
...
fn calculate_stroke_width_optimized(image: &ImageBuffer<Rgba<u8>, Vec<u8>>, base_stroke: f32) -> f32 {
    let (width, height) = image.dimensions();
    let diagonal = ((width * width + height * height) as f32).sqrt();
