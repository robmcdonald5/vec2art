...
impl DistanceTransformStrategy for FelzenszwalbHuttenlocher {
    fn compute_distance_transform(
        &self,
        binary: &GrayImage,
    ) -> Result<Vec<Vec<f32>>, VectorizeError> {
...
impl DistanceFieldCenterlineExtractor {
        ...
        for y in 1..height - 1 {
            for x in 1..width - 1 {
                ...
                let neighbors = [
                    ...
                    dt[y][x + 1], // horizontal
                    dt[y - 1][x - 1],
                    dt[y + 1][x + 1], // diagonal 1
                    dt[y - 1][x + 1],
                    dt[y + 1][x - 1], // diagonal 2
                ];
    ...
    fn trace_ridge_paths(
        ...
        for (start_x, start_y, _) in start_points {
            ...
            if path.len() >= 3 {
                // Only keep paths with reasonable length
                paths.push(path);
            }
        }
...
fn distance_transform_1d(input: &[f32]) -> Vec<f32> {
    ...
    for q in 1..n {
        ...
        v[k] = q;
        z[k] = if k < n {
            ((input[q] + q as f32 * q as f32)
                - (input[v[k - 1]] + v[k - 1] as f32 * v[k - 1] as f32))
...
fn simd_felzenszwalb_huttenlocher_dt(binary: &GrayImage) -> Vec<Vec<f32>> {
    use crate::algorithms::visual::simd_color::is_simd_available;
...
fn simd_initialize_distance_field(
    binary: &GrayImage,
    dt: &mut [Vec<f32>],
    width: usize,
    height: usize,
...
fn simd_distance_transform_1d(input: &[f32]) -> Vec<f32> {
    ...
    if n < 16 || !crate::algorithms::visual::simd_color::is_simd_available() {
