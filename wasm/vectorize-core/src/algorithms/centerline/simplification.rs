...
impl SimplificationStrategy for CurvatureAwareSimplification {
    fn simplify_paths(
        ...
        let simplified = paths.into_iter()
            .map(|path| curvature_aware_douglas_peucker(&path, base_epsilon * multiplier, sensitivity))
...
fn curvature_aware_douglas_peucker(path: &[Point], base_epsilon: f32, curvature_sensitivity: f32) -> Vec<Point> {
    if path.len() <= 2 { 
        return path.to_vec(); 
    }
    ...
    let normalized_curvatures: Vec<f32> = curvatures.iter()
...
fn curvature_aware_dp_recursive(
    ...
    if max_distance > 1.0 {
        curvature_aware_dp_recursive(path, curvatures, start, max_index, base_epsilon, curvature_sensitivity, result);
        result.push(path[max_index]);
        curvature_aware_dp_recursive(path, curvatures, max_index, end, base_epsilon, curvature_sensitivity, result);
    }
}
