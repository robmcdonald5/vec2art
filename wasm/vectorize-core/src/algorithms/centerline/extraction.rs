...
impl ExtractionStrategy for DirectionAwareExtraction {
    fn extract_polylines(&self, skeleton: &GrayImage) -> Result<Vec<Vec<Point>>, VectorizeError> {
        Ok(extract_direction_aware_polylines(skeleton, self.junction_priority_bias))
    }
...
fn extract_skeleton_polylines_improved(skeleton: &GrayImage) -> Vec<Vec<Point>> {
    ...
    for y in 0..height {
        for x in 0..width {
            if !visited[y as usize][x as usize] && pixel_types[y as usize][x as usize] == PixelType::Endpoint {
...
fn extract_direction_aware_polylines(skeleton: &GrayImage, junction_bias: f32) -> Vec<Vec<Point>> {
    ...
    for y in 0..height {
        for x in 0..width {
            if !visited[y as usize][x as usize] && pixel_types[y as usize][x as usize] == PixelType::Endpoint {
                let polyline = trace_direction_aware(skeleton, &mut visited, &pixel_types, x, y, junction_bias);
                if polyline.len() >= 2 {
                    polylines.push(polyline);
                }
            }
    ...
    loop {
        ...
        for dy in -1..=1 {
            for dx in -1..=1 {
                ...
                if !in_bounds(nx, ny) || !is_skeleton(nx, ny) { continue; }
        ...
        let next = if let Some((prev_dx, prev_dy)) = previous_direction {
            ...
            candidates.into_iter()
                .max_by(|a, b| {
                    ...
                    let junction_penalty_a = if pixel_types[a.1 as usize][a.0 as usize] == PixelType::Junction {
                        junction_bias
                    } else { 0.0 };
                    let junction_penalty_b = if pixel_types[b.1 as usize][b.0 as usize] == PixelType::Junction {
                        junction_bias  
                    } else { 0.0 };
    ...
    while let Some((x, y)) = queue.pop_front() {
        ...
        for dy in -1..=1 {
            for dx in -1..=1 {
                ...
                if nx >= 0 && ny >= 0 && (nx as u32) < width && (ny as u32) < height {
                    if skeleton.get_pixel(nx as u32, ny as u32).0[0] > 0 && !visited[ny as usize][nx as usize] {
