...
impl BradleyRothThresholding {
    pub fn new(window_size: u32, threshold_ratio: f32) -> Self {
        Self { window_size, threshold_ratio }
    }
}
...
impl ThresholdingStrategy for OtsuThresholding {
    fn threshold(&self, gray: &GrayImage, _config: &TraceLowConfig) -> Result<GrayImage, VectorizeError> {
...
fn otsu_threshold(gray: &GrayImage) -> GrayImage {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let binary_val = if pixel_val > optimal_threshold { 255 } else { 0 };
            binary.put_pixel(x, y, Luma([binary_val]));
        }
    }
...
fn compute_integral_image(gray: &GrayImage) -> Vec<Vec<u64>> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let left = if x > 0 { integral[y as usize][(x - 1) as usize] } else { 0 };
            let top = if y > 0 { integral[(y - 1) as usize][x as usize] } else { 0 };
