...
pub fn compute_etf(gray: &GrayImage, cfg: &EtfConfig) -> EtfField {
    ...
    log::debug!(
        "Computing ETF field for {}x{} image with config: {:?}",
        width,
        height,
        cfg
    );
...
fn compute_gradients(gray: &GrayImage) -> (Vec<f32>, Vec<f32>) {
    ...
    // Parallel computation of gradients using simple iteration
    let gradient_results: Vec<(f32, f32)> = (0..(width * height))
        .into_par_iter()
        .map(|i| {
            let y = i / width;
            let x = i % width;
...
fn compute_structure_tensor(
    ...
    let tensors: Vec<StructureTensor> = (0..size)
        .into_par_iter()
        .map(|i| {
            let gx = grad_x[i];
            let gy = grad_y[i];
...
fn gaussian_smooth_tensor(
    ...
    // Horizontal pass
    let size = (width * height) as usize;
    let temp_field: Vec<StructureTensor> = (0..size)
        .into_par_iter()
        .map(|i| {
    ...
    // Vertical pass
    let final_field: Vec<StructureTensor> = (0..size)
        .into_par_iter()
        .map(|i| {
            let y = i as u32 / width;
            let x = i as u32 % width;
...
fn refine_etf_field(etf_field: &mut EtfField, radius: u32, iters: u32) {
    ...
    for iteration in 0..iters {
        ...
        // Create temporary buffers for this iteration using parallel computation
        let size = (width * height) as usize;
        let refined_tangents: Vec<(f32, f32)> = (0..size)
            .into_par_iter()
            .map(|i| {
                let y = i as u32 / width;
                let x = i as u32 % width;
