...
impl GpuCannyEdgeDetector {
    ...
    pub fn new(device: Arc<GpuDevice>) -> Result<Self, GpuEdgeDetectionError> {
        // Load shaders
        let gaussian_shader = device.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Gaussian Blur Shader"),
            source: wgpu::ShaderSource::Wgsl(super::GAUSSIAN_BLUR_SHADER.into()),
        });
        
        let sobel_shader = device.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Sobel Edge Shader"),
            source: wgpu::ShaderSource::Wgsl(super::SOBEL_EDGE_SHADER.into()),
        });
        
        let nms_shader = device.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        let gaussian_pipeline = device.device.create_compute_pipeline(&ComputePipelineDescriptor {
        ...
        let sobel_pipeline = device.device.create_compute_pipeline(&ComputePipelineDescriptor {
        ...
        let nms_pipeline = device.device.create_compute_pipeline(&ComputePipelineDescriptor {
        ...
        let hysteresis_pipeline = device.device.create_compute_pipeline(&ComputePipelineDescriptor {
    ...
    pub async fn detect_edges(
        ...
        // Create GPU buffers
        let _input_buffer = self.device.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let _params_buffer = self.device.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        encoder.copy_buffer_to_buffer(&edges_buffer, 0, &staging_buffer, 0, (image_size * 4) as u64);
        ...
        rx.await
            .map_err(|_| GpuEdgeDetectionError::ExecutionFailed("Buffer mapping cancelled".to_string()))?
            .map_err(|e| GpuEdgeDetectionError::ExecutionFailed(format!("Buffer mapping failed: {:?}", e)))?;
