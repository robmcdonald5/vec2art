            ...
            if y < height && x < width {
                ...
                if distance_squared > (radius * radius) as f32 && 
                   distance_squared < ((radius + 5) * (radius + 5)) as f32 {
                    if (x + y) % 3 == 0 { // Sparse noise pattern
...
pub fn test_otsu_background_removal() -> VectorizeResult<BackgroundRemovalResult> {
    ...
    println!("✅ OTSU result: algorithm {:?}, threshold {}, took {}ms", 
...
pub fn test_adaptive_background_removal() -> VectorizeResult<BackgroundRemovalResult> {
    ...
    println!("✅ Adaptive result: algorithm {:?}, threshold {}, took {}ms",
             result.algorithm_used, result.threshold_used, result.processing_time_ms);
...
pub fn test_auto_background_removal() -> VectorizeResult<BackgroundRemovalResult> {
    ...
    println!("✅ Auto result: selected {:?}, threshold {}, took {}ms",
             result.algorithm_used, result.threshold_used, result.processing_time_ms);
...
pub fn analyze_background_removal_effect(original: &RgbaImage, processed: &RgbaImage) -> (u32, u32, f32) {
...
pub fn run_comprehensive_test() -> VectorizeResult<()> {
    ...
    println!("📷 Created test image: {}x{} pixels", original_image.width(), original_image.height());
    
    // Test OTSU
    let otsu_result = test_otsu_background_removal()?;
    let (otsu_transparent, otsu_changed, otsu_percent) = analyze_background_removal_effect(&original_image, &otsu_result.image);
    println!("📊 OTSU: {} transparent pixels ({:.1}%), {} changed pixels", 
             otsu_transparent, otsu_percent, otsu_changed);
    
    // Test Adaptive
    let adaptive_result = test_adaptive_background_removal()?;
    let (adaptive_transparent, adaptive_changed, adaptive_percent) = analyze_background_removal_effect(&original_image, &adaptive_result.image);
    println!("📊 Adaptive: {} transparent pixels ({:.1}%), {} changed pixels", 
             adaptive_transparent, adaptive_percent, adaptive_changed);
    
    // Test Auto
    let auto_result = test_auto_background_removal()?;
    let (auto_transparent, auto_changed, auto_percent) = analyze_background_removal_effect(&original_image, &auto_result.image);
    println!("📊 Auto: {} transparent pixels ({:.1}%), {} changed pixels",
    ...
    if otsu_transparent == 0 && adaptive_transparent == 0 && auto_transparent == 0 {
        println!("❌ ERROR: No transparent pixels created - background removal may not be working!");
    ...
    for strength in [0.1, 0.5, 0.9] {
        ...
        let (transparent, _, percent) = analyze_background_removal_effect(&original_image, &result.image);
        
        println!("🎛️ Strength {:.1}: {} transparent pixels ({:.1}%)", 
...
mod tests {
    ...
    fn test_background_removal_comprehensive() {
        ...
        assert!(result.is_ok(), "Background removal comprehensive test failed: {:?}", result.err());
    }
