...
pub fn apply_background_removal(
    ...
    log::debug!("Starting background removal with algorithm {:?}, strength {:.2}", 
                config.algorithm, config.strength);
    ...
    log::debug!("Using algorithm {:?} for background removal", algorithm_to_use);
    ...
    log::debug!("Background removal completed in {}ms using threshold {}", 
                processing_time_ms, threshold_used);
...
fn determine_best_algorithm(image: &RgbaImage) -> VectorizeResult<BackgroundRemovalAlgorithm> {
    ...
    let mean = brightness_values.iter().map(|&x| x as f32).sum::<f32>() / brightness_values.len() as f32;
    let variance = brightness_values.iter()
        .map(|&x| (x as f32 - mean).powi(2))
        .sum::<f32>() / brightness_values.len() as f32;
    let std_dev = variance.sqrt();
    
    log::debug!("Image analysis: mean brightness {:.1}, std dev {:.1}", mean, std_dev);
...
fn apply_otsu_background_removal(
    ...
    for (i, pixel) in image.pixels().enumerate() {
        grayscale[i] = (0.299 * pixel[0] as f32 + 
                       0.587 * pixel[1] as f32 + 
                       0.114 * pixel[2] as f32) as u8;
    }
...
fn apply_adaptive_background_removal(
    ...
    for y in 0..height {
        for x in 0..width {
            let threshold = calculate_local_threshold(&grayscale, x, y, window_size, config.strength);
...
fn calculate_otsu_threshold(grayscale: &[u8], strength: f32) -> VectorizeResult<u8> {
    ...
    for threshold in 0..256 {
        ...
        let between_class_variance = (background_pixels as f32) * (foreground_pixels as f32) * 
...
fn calculate_local_threshold(
    ...
    let x_min = if x >= half_window { x - half_window } else { 0 };
    let x_max = (x + half_window + 1).min(width);
    let y_min = if y >= half_window { y - half_window } else { 0 };
    ...
    let adjusted_threshold = (mean * strength_factor) as u8;
...
mod tests {
    ...
    fn test_adaptive_window_size_calculation() {
        ...
        assert!(window_size >= 15 && window_size <= 51, 
                "Window size {} should be in range 15-51", window_size);
    }
