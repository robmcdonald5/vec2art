...
use vectorize_core::algorithms::{
    ...
    apply_style_preset, DotConfig, DotStyle,
    dots_to_svg_with_config, SvgDotConfig,
    TraceBackend, TraceLowConfig,
};
...
mod image_generators {
    ...
    pub fn create_landscape_image(width: u32, height: u32) -> RgbaImage {
        ImageBuffer::from_fn(width, height, |x, y| {
            ...
            let mountain_height = (((x as f32 / width as f32) * 6.28).sin() * 20.0 + 
                                 ((x as f32 / width as f32) * 12.56).sin() * 10.0) + horizon - 40.0;
            ...
            if (y as f32) < mountain_height {
                ...
                Rgba([mountain_shade, mountain_shade + 20, mountain_shade - 10, 255])
            } else {
                // Ground with texture
                let ground_texture = ((x + y) % 7) as u8 * 15;
                Rgba([34 + ground_texture, 139 + ground_texture / 2, 34 + ground_texture / 3, 255])
            }
        })
    }
    ...
    pub fn create_logo_image(width: u32, height: u32) -> RgbaImage {
        ImageBuffer::from_fn(width, height, |x, y| {
            ...
            let circle_distance = ((x as f32 - center_x).powi(2) + (y as f32 - center_y).powi(2)).sqrt();
...
mod portrait_tests {
    ...
    fn test_portrait_processing() {
        ...
        for style in &styles {
            ...
            assert!(!dots.is_empty(), "Portrait should generate dots with {:?} style", style);
            
            // Portraits should have reasonable dot density
            let dot_density = dots.len() as f32 / (128.0 * 128.0);
            assert!(dot_density > 0.001 && dot_density < 0.3, 
                   "Portrait dot density should be reasonable: {} for style {:?}", dot_density, style);
        }
    }
    ...
    fn test_portrait_adaptive_density() {
        ...
        let dots = generate_dots_from_image(&img, &config, None, None);
        assert!(!dots.is_empty(), "Portrait should work with adaptive density");
        ...
        assert!(face_density >= 0.0 && corner_density >= 0.0, 
               "Both areas should have non-negative density: face={}, corner={}", 
               face_density, corner_density);
    }
    ...
    fn test_portrait_color_preservation() {
        ...
        assert!(has_skin_tone || has_hair_color || dots.len() > 10, 
               "Should preserve some original colors or generate reasonable dots");
    }

    fn count_dots_in_region(dots: &[vectorize_core::algorithms::Dot], x_min: f32, y_min: f32, x_max: f32, y_max: f32) -> usize {
        dots.iter()
...
mod landscape_tests {
    ...
    fn test_landscape_processing() {
        ...
        for style in &styles {
            ...
            let dots = generate_dots_from_image(&img, &config, None, None);
            assert!(!dots.is_empty(), "Landscape should generate dots with {:?} style", style);
            
            // Landscapes typically have varied density
            let total_area = 160.0 * 120.0;
            let dot_density = dots.len() as f32 / total_area;
            assert!(dot_density > 0.001, "Landscape should have reasonable dot density: {}", dot_density);
        }
    }
    ...
    fn test_landscape_region_analysis() {
        ...
        let dots = generate_dots_from_image(&img, &config, None, None);
        assert!(!dots.is_empty(), "Landscape should work with adaptive processing");
        ...
        assert!(sky_density >= 0.0 && ground_density >= 0.0, 
               "Both sky and ground should have valid density: sky={}, ground={}", 
               sky_density, ground_density);
    }
    ...
    fn test_landscape_performance() {
        ...
        assert!(elapsed.as_millis() < 400, "Landscape processing took {}ms, should be under 400ms", elapsed.as_millis());
...
mod logo_graphics_tests {
    ...
    fn test_logo_technical_drawing_style() {
        ...
        let dots = generate_dots_from_image(&img, &config, None, None);
        assert!(!dots.is_empty(), "Logo should work with technical drawing style");
    ...
    fn test_technical_drawing_processing() {
        ...
        for style in &styles {
            ...
            let dots = generate_dots_from_image(&img, &config, None, None);
            assert!(!dots.is_empty(), "Technical drawing should generate dots with {:?} style", style);
            
            // Technical drawings should capture line details
            let line_area_dots = count_dots_in_region(&dots, 0.0, 0.0, 96.0, 96.0);
            assert!(line_area_dots > 0, "Should capture technical drawing elements");
        }
    }
    ...
    fn test_logo_svg_output_quality() {
        ...
        assert!(svg.contains("circle") || svg.contains("path"), "SVG should contain drawing elements");
    }
...
mod edge_case_tests {
    ...
    fn test_solid_color_images() {
        ...
        for color in &colors {
            ...
            assert!(dot_density <= 0.1, "Solid color should have low dot density: {} for {:?}", dot_density, color);
        }
    }
    ...
    fn test_high_contrast_image() {
        ...
        for dot in &dots {
            assert!(dot.radius > 0.0 && dot.radius < 10.0, "Dot radius should be reasonable in high contrast");
            assert!(dot.opacity >= 0.0 && dot.opacity <= 1.0);
        }
    }
    ...
    fn test_low_contrast_image() {
        ...
        assert!(dot_density >= 0.001, "Low contrast should still generate some dots: {}", dot_density);
    }
    ...
    fn test_gradient_transitions() {
        ...
        let unique_colors: std::collections::HashSet<String> = dots.iter().map(|d| d.color.clone()).collect();
        assert!(unique_colors.len() > 1, "Gradient should produce varied colors: {} unique", unique_colors.len());
    }
    ...
    fn test_large_image_scalability() {
        ...
        assert!(elapsed.as_millis() < 1000, "Large image processing took {}ms, should be under 1000ms", elapsed.as_millis());
        ...
        assert!(processing_rate > 100.0, "Processing rate should be reasonable: {} pixels/ms", processing_rate);
    }
    ...
    fn test_aspect_ratio_variations() {
        ...
        for (width, height) in &aspect_ratios {
            ...
            assert!(!dots.is_empty(), "{}x{} image should generate dots", width, height);
            
            // Dots should be within image bounds
            for dot in &dots {
                assert!(dot.x >= 0.0 && dot.x < *width as f32, "Dot x should be in bounds for {}x{}", width, height);
                assert!(dot.y >= 0.0 && dot.y < *height as f32, "Dot y should be in bounds for {}x{}", width, height);
            }
        }
    }
    ...
    fn test_memory_efficiency_with_large_dots() {
        ...
        assert!(processing_time.as_millis() < 500, "High-density dot generation should complete in reasonable time");
        
        // Even with many dots, should not exceed memory limits
        let max_expected_dots = 128 * 128 / 4; // Rough upper bound
        assert!(dots.len() < max_expected_dots, "Should not generate excessive dots: {} (max expected: {})", dots.len(), max_expected_dots);
    }
...
mod integration_pipeline_tests {
    ...
    fn test_end_to_end_dot_pipeline() {
        ...
        for (name, img) in &test_images {
            ...
            assert!(elapsed.as_millis() < 200, "{} processing took {}ms, should be under 200ms", name, elapsed.as_millis());
        }
    }
    ...
    fn test_style_consistency_across_images() {
        ...
        for style in &styles {
            ...
            let dots = generate_dots_from_image(&img, &config, None, None);
            assert!(!dots.is_empty(), "Style {:?} should work consistently", style);
            ...
            assert!(avg_radius > 0.0 && avg_radius < 10.0, "Style {:?} should have reasonable average radius: {}", style, avg_radius);
            assert!(avg_opacity >= 0.0 && avg_opacity <= 1.0, "Style {:?} should have valid average opacity: {}", style, avg_opacity);
        }
    }
    ...
    fn test_parameter_boundary_conditions() {
        ...
        for (i, config) in boundary_configs.iter().enumerate() {
            ...
            if let Ok(dots) = result {
                ...
                for dot in &dots {
                    assert!(dot.radius >= config.min_radius && dot.radius <= config.max_radius,
                           "Boundary config {} should respect radius limits", i);
                    assert!(dot.opacity >= 0.0 && dot.opacity <= 1.0,
                           "Boundary config {} should maintain valid opacity", i);
                }
            }
        }
    }
    ...
    fn test_robustness_with_corrupted_inputs() {
        ...
        for (i, img) in edge_case_images.iter().enumerate() {
            ...
            assert!(result.is_ok(), "Edge case image {} should not cause panic", i);
