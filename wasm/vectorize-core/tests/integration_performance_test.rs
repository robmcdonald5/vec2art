...
fn create_performance_test_image(width: u32, height: u32) -> RgbaImage {
    ...
    add_rectangle(&mut img, width / 3, height / 2, width / 3, height / 4, Rgba([0, 0, 255, 255]));
...
fn test_performance_optimization_integration() {
    ...
    let improvement_ratio = standard_time.as_millis() as f64 / optimized_time.as_millis().max(1) as f64;
    ...
    let start = Instant::now();
    let profiled_dots = generator.generate_dots_optimized(&img, &gradient_analysis, &background_mask);
    ...
    if let Some(report) = generator.get_performance_report() {
        ...
        println!("  Memory usage: {:.2}MB", report.total_memory as f64 / 1_048_576.0);
...
fn test_scalability_performance() {
    ...
    assert!(results.last().unwrap().1 < 2000, "Largest image should complete in <2s");
...
fn test_optimization_components() {
    ...
    let configurations = vec![
        ...
        ("memory_pooling", OptimizedDotConfig {
            performance_config: PerformanceConfig {
                use_memory_pooling: true,
                use_simd: false,
                use_spatial_indexing: false,
        ...
        ("spatial_indexing", OptimizedDotConfig {
            performance_config: PerformanceConfig {
                use_memory_pooling: false,
                use_simd: false,
                use_spatial_indexing: true,
        ...
        ("all_optimizations", OptimizedDotConfig {
            performance_config: PerformanceConfig {
                use_memory_pooling: true,
                use_simd: true,
                use_spatial_indexing: true,
    ...
    for (name, config) in configurations {
        ...
        println!("{}: {:?}ms, {} dots", name, duration.as_millis(), dots.len());
...
fn test_performance_consistency() {
    ...
    for i in 0..5 {
        ...
        println!("Run {}: {:?}ms, {} dots", i + 1, duration.as_millis(), dots.len());
    }
    ...
    println!("  Duration: avg={}ms, variance={}ms ({:.1}%)",
        avg_duration, duration_variance,
        (duration_variance as f64 / avg_duration as f64) * 100.0
    );
    println!("  Dot count: avg={}, variance={} ({:.1}%)",
        avg_dots, dot_variance,
