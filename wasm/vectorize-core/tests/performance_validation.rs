...
use vectorize_core::performance::{
    ...
    simd_ops::{simd_gradient_magnitude, simd_color_distance, get_available_simd_features},
...
fn create_test_image(width: u32, height: u32, complexity: ImageComplexity) -> RgbaImage {
    ...
    match complexity {
        ...
        ImageComplexity::Medium => {
            ...
            add_circle(&mut img, 3*width/4, height/4, 25, Rgba([0, 255, 0, 255]));
            add_circle(&mut img, width/2, 3*height/4, 40, Rgba([0, 0, 255, 255]));
        },
...
impl PerformanceMeasurement {
    fn new(duration: Duration, dots_generated: usize, memory_usage_bytes: usize, configuration: String) -> Self {
...
fn test_performance_target_500x500() {
    ...
    let optimized_result = measure_optimized_dot_generation_performance(&img, &optimized_config, "optimized");
    
    println!("500x500 Performance Results:");
    println!("Standard: {:?}ms, {} dots, {:.1} dots/sec", 
        standard_result.duration.as_millis(),
        standard_result.dots_generated,
        standard_result.dots_per_second
    );
    println!("Optimized: {:?}ms, {} dots, {:.1} dots/sec",
    ...
    let memory_per_dot = optimized_result.memory_usage_bytes / optimized_result.dots_generated.max(1);
    assert!(
...
fn test_performance_target_1000x1000() {
    ...
    let optimized_config = OptimizedDotConfig::default();
    let result = measure_optimized_dot_generation_performance(&img, &optimized_config, "optimized_1000x1000");
    
    println!("1000x1000 Performance Result:");
    println!("Time: {:?}ms, {} dots, {:.1} dots/sec", 
...
fn test_gradient_calculation_performance() {
    ...
    for (i, &(width, height)) in sizes.iter().enumerate() {
        ...
        let start = Instant::now();
        let _simd_magnitudes = simd_gradient_magnitude(gray.as_raw(), width as usize, height as usize);
        let simd_time = start.elapsed();
        
        println!("Gradient calculation {}x{}: standard={}ms, simd={}ms", 
            width, height, standard_time.as_millis(), simd_time.as_millis());
        
        // Validate performance targets
        assert!(
            standard_time.as_millis() <= target_times[i] as u128,
            "Gradient calculation for {}x{} took {}ms, expected <{}ms",
            width, height, standard_time.as_millis(), target_times[i]
        );
...
fn test_spatial_indexing_performance() {
    ...
    for &dot_count in &dot_counts {
        ...
        println!("Spatial indexing with {} dots: {}ms", dot_count, duration.as_millis());
        ...
        assert!(
            ...
            dot_count, duration.as_millis(), (dot_count as f64 * max_time_per_dot) as u128
        );
    }
}
    ...
    for &count in &allocation_counts {
        ...
        for i in 0..count {
            let dot = vectorize_core::algorithms::dots::Dot::new(
                i as f32, i as f32, 2.0, 1.0, "#000000".to_string()
            );
        ...
        println!("Memory pooling {} allocations: baseline={}ms, pooled={}ms", 
            count, baseline_time.as_millis(), pooled_time.as_millis());
        
        // Pooling should be faster or at least comparable
        let pool_stats = pool.stats();
        assert!(pool_stats.hit_ratio > 0.0, "Memory pool should have some reuse");
        
        // For large allocation counts, pooling should show benefits
        if count >= 5000 {
            let improvement_ratio = baseline_time.as_millis() as f64 / pooled_time.as_millis().max(1) as f64;
            assert!(
...
fn test_simd_operations_performance() {
    ...
    println!("Available SIMD features: {:?}", get_available_simd_features());
    
    for &size in &sizes {
        // Test color distance calculation
        let colors1: Vec<(u8, u8, u8)> = (0..size)
            .map(|i| ((i % 255) as u8, ((i * 2) % 255) as u8, ((i * 3) % 255) as u8))
            .collect();
        let colors2: Vec<(u8, u8, u8)> = (0..size)
            .map(|i| (((i + 50) % 255) as u8, ((i * 2 + 100) % 255) as u8, ((i * 3 + 150) % 255) as u8))
            .collect();
        ...
        println!("SIMD color distance {} items: {}ms", size, duration.as_millis());
        ...
        assert!(
            ...
            size, duration.as_millis(), (size as f64 * max_time_per_item) as u128
        );
    }
}
...
fn test_parallel_processing_scaling() {
    ...
    for &(width, height) in &sizes {
        ...
        println!("Parallel processing {}x{}: {}ms", width, height, duration.as_millis());
...
fn test_optimization_effectiveness() {
    ...
    let standard_result = measure_optimized_dot_generation_performance(
        &img, &standard_config, "standard"
    );
    let optimized_result = measure_optimized_dot_generation_performance(
        &img, &optimized_config, "optimized"
    );
    
    println!("Optimization effectiveness:");
    println!("Standard: {}ms, {} dots", standard_result.duration.as_millis(), standard_result.dots_generated);
    println!("Optimized: {}ms, {} dots", optimized_result.duration.as_millis(), optimized_result.dots_generated);
    
    // Optimizations should not significantly change dot count
    let dot_count_diff = (standard_result.dots_generated as i32 - optimized_result.dots_generated as i32).abs();
    ...
    assert!(
        ...
        standard_result.dots_generated, optimized_result.dots_generated, dot_count_diff
    );
    
    // Optimizations should improve or maintain performance
    let performance_ratio = optimized_result.duration.as_millis() as f64 / standard_result.duration.as_millis().max(1) as f64;
    assert!(
    ...
    println!("Performance ratio (optimized/standard): {:.2}x", performance_ratio);
}
...
fn test_memory_usage_efficiency() {
    ...
    let estimated_memory = PerformanceUtils::estimate_memory_usage(
        img.width(), img.height(), estimated_dots, &config.performance_config
    );
    
    let result = measure_optimized_dot_generation_performance(&img, &config, "memory_test");
    
    println!("Memory usage: estimated={}KB, actual={}KB", 
        estimated_memory / 1024, result.memory_usage_bytes / 1024);
    ...
    assert!(
        ...
        result.memory_usage_bytes / 1024, estimated_memory / 1024, memory_ratio
    );
}
...
fn test_regression_protection() {
    ...
    for i in 0..5 {
        let result = measure_optimized_dot_generation_performance(&img, &config, &format!("measurement_{}", i));
        measurements.push(result);
    }
    
    let avg_time = measurements.iter().map(|m| m.duration.as_millis()).sum::<u128>() / measurements.len() as u128;
    let avg_dots = measurements.iter().map(|m| m.dots_generated).sum::<usize>() / measurements.len();
    
    println!("Regression test - baseline: {}ms, average: {}ms", baseline.duration.as_millis(), avg_time);
    
    // Performance should not regress significantly
    let test_config = PerformanceTestConfig::default();
    let regression_threshold = baseline.duration.as_millis() as f64 * (1.0 + test_config.regression_tolerance / 100.0);
    
    assert!(
        avg_time as f64 <= regression_threshold,
        "Performance regression detected: {}ms average vs {}ms baseline (threshold: {}ms)",
        avg_time, baseline.duration.as_millis(), regression_threshold as u128
    );
    
    // Dot count should be consistent
    let dot_variance = measurements.iter()
