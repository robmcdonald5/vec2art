...
fn test_max_image_size_parameter() {
    ...
    assert!(builder_result.is_ok(), "Should be able to set max_image_size to 2048");
    ...
    assert!(builder_result.is_err(), "Should reject max_image_size < 512");

    // Test validation - too large
    let builder_result = ConfigBuilder::new().max_image_size(16384);
    assert!(builder_result.is_err(), "Should reject max_image_size > 8192");
...
fn test_svg_precision_parameter() {
    ...
    assert!(builder_result.is_ok(), "Should be able to set svg_precision to 3");
...
fn test_image_resizing_functionality() {
    ...
    assert!(!svg.is_empty(), "Should produce SVG output even after resizing");
...
fn test_svg_precision_in_output() {
    ...
    assert!(svg_int.contains("<svg"), "Integer precision SVG should be valid");
    assert!(svg_precise.contains("<svg"), "High precision SVG should be valid");
}

#[test]
fn test_default_values() {
    let config = TraceLowConfig::default();
    assert_eq!(config.max_image_size, 4096, "Default max_image_size should be 4096");
