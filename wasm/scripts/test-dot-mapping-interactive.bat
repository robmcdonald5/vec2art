...
if /i "%1"=="--all-dots" (
    ...
    echo.
    echo Usage: test-dot-mapping-interactive.bat [OPTIONS]
    ...
    echo.
    echo Examples:
    echo   scripts\test-dot-mapping-interactive.bat
    echo   scripts\test-dot-mapping-interactive.bat --all-dots
    echo   scripts\test-dot-mapping-interactive.bat --all-line
    echo.
    pause
    exit /b 0
) else if not "%1"=="" (
...
if "!AUTO_MODE!"=="true" (
    ...
    echo  VEC2ART INTERACTIVE DOT MAPPING TEST SUITE
)
echo ================================================================================
if "!AUTO_MODE!"=="true" (
...
echo - Output directory: examples\outputs\dot_mapping\
...
if "!AUTO_MODE!" neq "true" (
    ...
    if /i "!openDir!"=="y" (
        start "" "examples\outputs\dot_mapping"
    )
)
echo.
if "!AUTO_MODE!"=="true" (
    ...
    echo Thank you for using the Interactive Dot Mapping Test Suite!
)
pause
exit /b 0
...
echo   "12MNACHJ" = Edge (Basic+High) + Dots (Ultra Fine+Medium) with options
echo.
...
if /i "!selections!"=="all" (
    echo.
    echo WARNING: ALL option will run 15+ comprehensive tests on this image.
    echo Estimated time: 5-10 minutes depending on image size and complexity.
    echo This includes:
    ...
    set "selections=12MNOPABCDEFGHIJKL"
    echo.
...
:: Check if edge backend is selected using string substitution
...
:: Debug output for automated mode
if "!AUTO_MODE!"=="true" (
    echo [DEBUG] edgeSelected: !edgeSelected!, dotsSelected: !dotsSelected!
)

if "!edgeSelected!"=="false" if "!dotsSelected!"=="false" (
    echo Error: You must select at least one backend (1 for Edge or 2 for Dots)
    if "!AUTO_MODE!"=="true" (
...
echo   "12MNCH"  = Both backends with multiple options
echo.
...
echo %selections% | findstr /i "M" >nul && (
    ...
    cargo run --release --bin vectorize-cli -- trace-low "%imagePath%" "examples\outputs\dot_mapping\%imageName%-!testCount!-edge-basic.svg" --backend edge --detail 0.3
)
...
echo %selections% | findstr /i "N" >nul && (
    ...
    cargo run --release --bin vectorize-cli -- trace-low "%imagePath%" "examples\outputs\dot_mapping\%imageName%-!testCount!-edge-detail.svg" --backend edge --detail 0.4
)
...
echo %selections% | findstr /i "O" >nul && (
    ...
    cargo run --release --bin vectorize-cli -- trace-low "%imagePath%" "examples\outputs\dot_mapping\%imageName%-!testCount!-edge-multipass.svg" --backend edge --detail 0.3 --multipass
)
...
echo %selections% | findstr /i "P" >nul && (
    ...
    cargo run --release --bin vectorize-cli -- trace-low "%imagePath%" "examples\outputs\dot_mapping\%imageName%-!testCount!-edge-directional.svg" --backend edge --detail 0.3 --multipass --enable-reverse --enable-diagonal
)
