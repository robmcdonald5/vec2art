...
pub fn sobel_edge_detection(image: &GrayImage) -> (GrayImage, GrayImage) {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let dir_normalized =
                ((dir + std::f32::consts::PI) / (2.0 * std::f32::consts::PI) * 255.0) as u8;
        ...
        for x in offset..(width as usize - offset) {
            ...
            for ky in 0..kernel_size {
                for kx in 0..kernel_size {
                    let px = image.get_pixel((x + kx - offset) as u32, (y + ky - offset) as u32)[0];
...
pub fn non_maximum_suppression(magnitude: &GrayImage, direction: &GrayImage) -> GrayImage {
    ...
    for y in 1..height - 1 {
        for x in 1..width - 1 {
            ...
            let dir = direction.get_pixel(x, y)[0] as f32 / 255.0 * 2.0 * std::f32::consts::PI
                - std::f32::consts::PI;

            // Quantize direction to 8 directions
            let dir_idx =
                ((dir + std::f32::consts::PI) / (std::f32::consts::PI / 4.0)).round() as usize % 8;
            ...
            let mag1 =
                magnitude.get_pixel((x as i32 + dx1) as u32, (y as i32 + dy1) as u32)[0] as f32;
            let mag2 =
                magnitude.get_pixel((x as i32 + dx2) as u32, (y as i32 + dy2) as u32)[0] as f32;
