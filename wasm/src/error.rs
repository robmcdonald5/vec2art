...
pub enum Vec2ArtError {
    ...
    #[error("Memory budget exhausted: requested {requested} MB, available {available} MB")]
    MemoryExhausted { requested: usize, available: usize },

    #[error("Memory limit exceeded: current usage {current} MB exceeds limit of {limit} MB")]
