use crate::algorithms::{SvgPath, utils};
        ...
        match params {
            ConversionParameters::EdgeDetector {
                ...
                let edges = match method {
                    EdgeMethod::Canny => {
                        canny_edge_detection_parallel(&gray, gaussian_sigma, threshold_low, threshold_high)?
                    }
                    EdgeMethod::Sobel => {
                        sobel_edge_detection_parallel(&gray, threshold_low)?
                    }
                };
                ...
                info!("Parallel edge detection complete, found {} paths", simplified_paths.len());
                Ok(svg)
            }
...
fn sobel_edge_detection_parallel_internal(image: &GrayImage) -> (GrayImage, GrayImage) {
    ...
    #[cfg(feature = "parallel")]
    {
        ...
        if capabilities.can_use_parallel_processing() {
            ...
            let chunk_size = calculate_optimal_chunk_size(row_data.len(), capabilities.recommended_thread_count());
...
fn hysteresis_threshold_parallel(image: &GrayImage, low: u8, high: u8) -> GrayImage {
    ...
    #[cfg(feature = "parallel")]
    {
        let capabilities = get_capabilities();
        if capabilities.can_use_parallel_processing() {
            let pixels: Vec<_> = (0..height).flat_map(|y| (0..width).map(move |x| (x, y))).collect();
            let chunk_size = calculate_optimal_chunk_size(pixels.len(), capabilities.recommended_thread_count());
...
/// Parallel path simplification using Douglas-Peucker algorithm
fn simplify_paths_parallel(paths: Vec<Vec<(f32, f32)>>, epsilon: f32) -> Vec<SvgPath> {
    let _timer = PerfTimer::new("Parallel path simplification");
    
    #[cfg(feature = "parallel")]
    {
        let capabilities = get_capabilities();
        if capabilities.can_use_parallel_processing() {
            let chunk_size = calculate_optimal_chunk_size(paths.len(), capabilities.recommended_thread_count());
            
            return paths
                .par_chunks(chunk_size)
                .map(|chunk| {
                    chunk.iter().map(|points| {
...
fn generate_svg(paths: &[SvgPath], width: u32, height: u32) -> String {
    let mut builder = SvgBuilder::new(width, height)
        .with_metadata("Vec2Art Parallel Edge Detection", "Parallel edge-detected vector graphics")
