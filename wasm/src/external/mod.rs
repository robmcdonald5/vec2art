...
pub use autotrace::{is_autotrace_available, AutotraceVectorizer};
pub use potrace::{is_potrace_available, PotraceVectorizer};

use crate::algorithms::SvgPath;
use crate::error::{Result, Vec2ArtError};
use crate::svg_builder::{generate_optimized_svg_from_paths, optimization_utils};
...
impl ExternalAlgorithms {
    ...
    pub fn convert_with_autotrace_outline(
        &self,
        image: DynamicImage,
        params: ConversionParameters,
    ) -> Result<Vec<SvgPath>> {
    ...
    pub fn convert_with_autotrace_centerline(
        &self,
        image: DynamicImage,
        params: ConversionParameters,
    ) -> Result<Vec<SvgPath>> {
    ...
    /// Convert image to optimized SVG using the best external algorithm
    pub fn convert_to_optimized_svg(
        &self,
        image: DynamicImage,
        params: ConversionParameters,
    ) -> Result<String> {
        let (width, height) = image.dimensions();
        let paths = self.convert_auto(image, params)?;
        
        // Log optimization statistics
        let complexity = optimization_utils::calculate_complexity_score(&paths);
        log::info!("Original paths: {}, Complexity score: {:.2}", paths.len(), complexity);
        
        // Generate optimized SVG
        let optimized_svg = generate_optimized_svg_from_paths(&paths, width, height);
            ...
            for x in (0..width).step_by(sample_rate as usize) {
                ...
                if x > 0 && y > 0 {
                    ...
                    let diff = ((pixel[0] as i32 - prev_pixel[0] as i32).abs()
                        ...
                        + (pixel[2] as i32 - prev_pixel[2] as i32).abs())
                        as u32;
                    if diff > 100 {
                        edge_pixels += 1;
                    }
