...
impl AutotraceVectorizer {
    ...
    pub fn vectorize(&self, image: DynamicImage, params: ConversionParameters) -> Result<Vec<SvgPath>> {
        ...
        // Perform tracing
        let spline_list = unsafe {
            ffi::autotrace_image(bitmap, options)
        };
        
        if spline_list.is_null() {
            self.cleanup_autotrace_bitmap(bitmap);
            self.cleanup_autotrace_options(options);
            return Err(Vec2ArtError::PathTracingError("Autotrace failed".to_string()));
        }
    ...
    fn create_autotrace_bitmap(&self, image: &RgbImage) -> Result<*mut ffi::autotrace_bitmap_t> {
        ...
        let bitmap = unsafe {
            ffi::autotrace_bitmap_new(width as i32, height as i32)
        };
    ...
    fn create_autotrace_options(&self, corner_threshold: f64, line_threshold: f64, 
                               despeckle: f64, centerline: bool) -> Result<*mut ffi::autotrace_options_t> {
    ...
    fn extract_paths_from_splines(&self, spline_list: *mut ffi::autotrace_spline_list_t) -> Result<Vec<SvgPath>> {
            ...
            while !current_spline.is_null() {
                ...
                if !path_data.is_empty() {
                    ...
                    svg_paths.push(SvgPath {
                        data: path_data,
                        fill: if self.centerline_mode { None } else { Some(color.clone()) },
                        stroke: if self.centerline_mode { Some(color) } else { None },
                        stroke_width,
                    });
                }
    ...
    fn convert_autotrace_spline_to_svg(&self, spline: *mut ffi::autotrace_spline_t) -> Result<String> {
            ...
            while !segment.is_null() {
                match (*segment).type_ {
                    ffi::autotrace_segment_type_t::AUTOTRACE_LINETO => {
                        if first_segment {
                            path_data.push_str(&format!("M {:.2} {:.2} ", 
                                (*segment).p1.x, (*segment).p1.y));
                            first_segment = false;
                        } else {
                            path_data.push_str(&format!("L {:.2} {:.2} ", 
                                (*segment).p1.x, (*segment).p1.y));
                        }
                    },
                    ffi::autotrace_segment_type_t::AUTOTRACE_CURVETO => {
                        ...
                        path_data.push_str(&format!("C {:.2} {:.2} {:.2} {:.2} {:.2} {:.2} ",
...
impl AutotraceVectorizer {
    ...
    pub fn vectorize(&self, _image: DynamicImage, _params: ConversionParameters) -> Result<Vec<SvgPath>> {
    ...
    fn test_bitmap_creation() {
        #[cfg(feature = "autotrace")]
        {
            ...
            let image = ImageBuffer::from_fn(50, 50, |x, y| {
                if x == y { Rgb([0, 0, 0]) } else { Rgb([255, 255, 255]) }
            });
