...
impl PotraceVectorizer {
    ...
    pub fn vectorize(&self, image: DynamicImage, params: ConversionParameters) -> Result<Vec<SvgPath>> {
        ...
        let (threshold, curve_smoothing, corner_threshold, optimize_curves) = match params {
            ConversionParameters::PathTracer { 
                ...
                optimize_curves, 
                .. 
            } => (threshold, curve_smoothing, corner_threshold, optimize_curves),
            _ => return Err(Vec2ArtError::InvalidParameters(
        ...
        // Set up parameters  
        let potrace_params = self.create_potrace_params(
            curve_smoothing,
            corner_threshold, 
            optimize_curves
        )?;
        ...
        // Perform tracing
        let state = unsafe {
            ffi::potrace_trace(potrace_params, bitmap)
        };
    ...
    fn create_potrace_bitmap(&self, image: &GrayImage) -> Result<*mut ffi::potrace_bitmap_t> {
        ...
        let bitmap = unsafe {
            ffi::potrace_bitmap_new(width as i32, height as i32)
        };
    ...
    fn create_potrace_params(&self, curve_smoothing: f32, corner_threshold: f32, optimize_curves: bool) -> Result<*mut ffi::potrace_param_t> {
                ...
                match (*segment).type_ {
                    ...
                    ffi::potrace_segment_type_t::POTRACE_CURVETO => {
                        ...
                        path_data.push_str(&format!("C {:.2} {:.2} {:.2} {:.2} {:.2} {:.2} ",
                            (*segment).c[0].x, (*segment).c[0].y,
                            (*segment).c[1].x, (*segment).c[1].y,
                            (*segment).c[2].x, (*segment).c[2].y));
                    },
                }
