...
use crate::{memory_monitor, ConversionParameters};
    ...
    fn convert(image: DynamicImage, params: ConversionParameters) -> Result<String> {
        ...
        match params {
            ConversionParameters::PathTracer {
                ...
                // Check memory for color quantization
                let quantize_size = (image.width() * image.height() * 3) as usize; // RGB
                ...
                memory_monitor::record_allocation(quantize_size, "quantized_image", "path_tracer");
...
fn quantize_image(image: &DynamicImage, num_colors: usize) -> Result<DynamicImage> {
    ...
    for (x, y, pixel) in rgb_image.enumerate_pixels() {
        ...
        let nearest = color_cache
            .entry(key)
            .or_insert_with(|| find_nearest_color_lab(pixel, &palette));
...
fn find_nearest_color_lab(pixel: &Rgb<u8>, palette: &[Rgb<u8>]) -> Rgb<u8> {
    ...
    for &color in palette {
        ...
        let dist = ((pixel_lab.l - color_lab.l).powi(2)
...
fn find_contours_filtered(image: &GrayImage, min_area: f32) -> Vec<Contour> {
    ...
    let filtered_contours: Vec<Contour> = contours
        .into_iter()
        .filter(|contour| {
    ...
    info!(
        "Filtered {} small contours, kept {} contours",
