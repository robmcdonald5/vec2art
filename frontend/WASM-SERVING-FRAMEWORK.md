...
<Modal bind:open={showInitDialog}>
	...
	<div class="quick-options">
		<button onclick={() => initWithThreads(1)}> 🔋 Low Power (1 thread) </button>
	...
	{#if showAdvanced}
		<div class="advanced-options">
			...
			<p class="hint">More threads = faster processing but higher CPU usage</p>
		</div>
	{/if}
...
class WasmErrorHandler {
	async handleInitError(error: Error): Promise<void> {
		if (error.message.includes('SharedArrayBuffer')) {
			...
			this.suggestLowerResources('Low memory detected. Try reducing thread count or image size.');
		} else {
...
```typescript
export async function loadVectorizer(options?: { threadCount?: number; autoStart?: boolean }) {
