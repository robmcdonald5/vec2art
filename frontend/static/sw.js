...
const STATIC_ASSETS = ['/gallery/manifest.json'];
		...
		caches.keys().then((cacheNames) => {
			return Promise.all(
				cacheNames
					.filter(
						(name) =>
							name.startsWith('vec2art-') && name !== CACHE_NAME && name !== IMAGE_CACHE_NAME
					)
					.map((name) => {
...
self.addEventListener('fetch', (event) => {
	...
	if (
		url.pathname.includes('/gallery/before/') ||
...
self.addEventListener('message', (event) => {
	...
	if (event.data.action === 'getCacheSize') {
		event.waitUntil(
			Promise.all([caches.open(CACHE_NAME), caches.open(IMAGE_CACHE_NAME)])
				.then(([cache, imageCache]) => {
					return Promise.all([cache.keys(), imageCache.keys()]);
				})
			...
			caches
				...
				.then((cache) => {
					...
					const imageUrls = items
						.flatMap((item) => [item.beforeImage, item.afterImage])
						.filter(Boolean);
