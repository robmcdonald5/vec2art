...
class PresetIssuesTester {
	...
	async getSuperpixelParameters() {
		...
		const numSuperpixels = await this.getParameterValue('Number of Superpixels', 'input[type="range"]');
	...
	async selectPresetByName(presetName: string) {
		...
		const presetOption = this.page.locator(`button:has-text("${presetName}")`, { hasText: presetName });
	...
	async getAvailablePresets() {
		...
		const presetTexts = await this.page.locator('[role="option"]:visible, button:visible').allTextContents();
		
		// Filter to actual preset names (remove UI elements)
		const presets = presetTexts
			.filter(text => text.trim().length > 0)
			.filter(text => !text.includes('Clear') && !text.includes('Convert') && !text.includes('Settings'));
