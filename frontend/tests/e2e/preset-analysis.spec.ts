...
class PresetAnalyzer {
	...
	async getAvailableAlgorithms() {
		...
		const algorithmOptions = await this.page.locator('button:visible, [role="option"]:visible').allTextContents();
	...
	async getAvailablePresets() {
		...
		const presetOptions = await this.page.locator('button:visible, [role="option"]:visible').allTextContents();
	...
	async selectAlgorithmByText(algorithmText: string) {
		...
		const algorithmOption = this.page.locator(`button:has-text("${algorithmText}"), [role="option"]:has-text("${algorithmText}")`).first();
	...
	async selectPresetByText(presetText: string) {
		...
		const presetOption = this.page.locator(`button:has-text("${presetText}"), [role="option"]:has-text("${presetText}")`).first();
	...
	async processImage() {
		...
		if (await convertButton.isVisible()) {
			...
			const downloadAvailable = await this.page.locator('button').filter({ hasText: 'Download' }).isVisible({ timeout: 5000 });
...
test.describe('Preset Analysis Suite', () => {
	...
	test('Test preset changes affect parameters', async ({ page }) => {
		...
		console.log('❌ Presets that DON\'T change parameters:', 
	...
	test('Test algorithm switching', async ({ page }) => {
		...
		// Test each algorithm
		for (const algorithm of availableAlgorithms.slice(0, 3)) { // Limit to avoid timeout
