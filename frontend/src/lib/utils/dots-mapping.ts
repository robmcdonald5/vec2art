...
export function mapDetailToDensity(
    ...
    if (imageWidth && imageHeight) {
        ...
        if (megapixels > 2.0) { // Images larger than 2MP
            const sizeMultiplier = Math.min(2.0, 1 + (megapixels - 2.0) * 0.15);
            baseDensity = Math.min(maxDensity, baseDensity * sizeMultiplier);
            console.log(`[Dots Mapping] 🖼️ Large image (${megapixels.toFixed(1)}MP) - adjusted density from ${(maxDensity - (clampedDetail * (maxDensity - minDensity))).toFixed(3)} to ${baseDensity.toFixed(3)}`);
        }
    }
...
export function mapUIConfigToDotsConfig(
    ...
    if (imageWidth && imageHeight) {
        ...
        if (megapixels > 3.0) {
            ...
            console.log(`[Dots Mapping] 🔍 Large image - adjusted dot sizes: ${minRadius.toFixed(1)}-${maxRadius.toFixed(1)} → ${adjustedMinRadius.toFixed(1)}-${adjustedMaxRadius.toFixed(1)}`);
        }
    }
...
export function validateDotsConfig(config: DotsBackendConfig): ConfigValidationResult {
    ...
    if (typeof config.dot_density_threshold !== 'number') {
        ...
        if (density < DOTS_PARAMETER_RANGES.DENSITY.ULTRA_FINE || 
