...
export function generatePerformanceRecommendations(
  ...
  recommendations.push({
    ...
    warnings: [],
    estimatedProcessingTime: capabilities.estimatedPerformance === 'high' ? '1-2 seconds' : '2-3 seconds',
  ...
  recommendations.push({
    ...
    estimatedProcessingTime: capabilities.estimatedPerformance === 'high' ? '0.8-1.5 seconds' : '1.5-2.5 seconds',
...
function estimatePerformanceClass(cores: number, deviceType: CPUCapabilities['deviceType']): CPUCapabilities['estimatedPerformance'] {
...
function calculateThreadRecommendations(
  ...
  // Desktop/laptop
  const recommended = performance === 'extreme' 
    ? Math.min(cores - 2, 12)
    : Math.min(cores - 2, 8);
