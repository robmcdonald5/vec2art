...
export class SpatialIndex<T = any> {
  ...
  queryViewport(viewport: BoundingBox): {
    ...
    const cullPercentage = this.itemCount > 0 ? 
      ((this.itemCount - visibleCount) / this.itemCount * 100) : 0;
  ...
  private splitNode(node: RTreeNode<T>): void {
    const items = node.isLeaf ? [...node.items] : [...node.children];
    ...
    // Linear split algorithm for simplicity
    const seeds = this.pickSeeds(items);
    const group1 = [seeds.item1];
    const group2 = [seeds.item2];

    const remaining = items.filter(item => item !== seeds.item1 && item !== seeds.item2);

    // Distribute remaining items
    for (const item of remaining) {
      const bounds = node.isLeaf ? (item as SpatialItem<T>).bounds : (item as RTreeNode<T>).bounds;
      
      const enlargement1 = this.getEnlargement(bounds, this.getBoundingBox(group1, node.isLeaf));
      const enlargement2 = this.getEnlargement(bounds, this.getBoundingBox(group2, node.isLeaf));
    ...
    if (node.isLeaf) {
      node.items = group1 as SpatialItem<T>[];
      const newNode = this.createNode([], true, node.level);
      newNode.items = group2 as SpatialItem<T>[];
      this.updateBounds(newNode);
    } else {
      node.children = group1 as RTreeNode<T>[];
      const newNode = this.createNode(group2 as RTreeNode<T>[], false, node.level);
      this.updateBounds(newNode);
    }
    ...
    if (node === this.root) {
      ...
      if (node.isLeaf) {
        ...
        newLeaf.items = group2 as SpatialItem<T>[];
        newRoot.children.push(newLeaf);
        this.updateBounds(newLeaf);
      } else {
        const newInternal = this.createNode(group2 as RTreeNode<T>[], false, node.level);
  ...
  private pickSeeds<U>(items: U[]): { item1: U; item2: U } {
    let maxSeparation = -1;
    let seed1 = items[0];
    let seed2 = items[1];
  ...
  private getBoundingBox<U>(items: U[], isItems: boolean): BoundingBox {
    ...
    for (let i = 1; i < items.length; i++) {
      const bounds = isItems ? (items[i] as SpatialItem<T>).bounds : (items[i] as RTreeNode<T>).bounds;
