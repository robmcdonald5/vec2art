...
{#if showTroubleshooting}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onclick={onClose}>
    <div class="mx-4 w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl dark:bg-gray-900" onclick={(e) => e.stopPropagation()}>
      ...
      <!-- Current Status -->
      <div class="mb-6 rounded-lg border p-4 {webglSupported || webgpuSupported ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}">
      ...
      <div class="space-y-4">
        ...
        <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
          ...
          <div class="text-sm space-y-2">
            <div><strong>Chrome/Edge:</strong> Settings → Advanced → System → "Use hardware acceleration when available"</div>
            <div><strong>Firefox:</strong> Preferences → General → Performance → Uncheck "Use recommended performance settings"</div>
          </div>
        </div>
        ...
        <div class="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
            ⚠️ <strong>Use with caution:</strong> This can cause browser crashes if your GPU has known issues.
          </p>
          <Button 
            variant="outline" 
            size="sm" 
      ...
      <div class="mt-6 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
        ...
        <p class="text-sm text-blue-800 dark:text-blue-200">
          Even without GPU acceleration, our new WebP conversion provides fast preview with full pan/zoom for large SVGs. 
          Your 1.5MB SVG will be converted to a compressed WebP image that loads quickly while maintaining visual quality.
        </p>
      </div>
