...
<script lang="ts">
      ...
      try {
        ...
        if (isHuge) {
          ...
          try {
            ...
            const analysisStrategy = svgPreviewRenderer.analyzeRenderStrategy(svgContent, backend, options);
  ...
  async function renderWithCanvas(options: PreviewRenderOptions) {
    ...
    console.log('[AdvancedSvgPreview] Canvas render result:', { hasDataUrl: !!result.dataUrl, hasCanvas: !!result.canvas });
  ...
  {#if showControls}
    <div class="mb-3 flex items-center justify-between px-2">
      ...
      <div class="flex items-center gap-2">        
        {#if isRendering}
          <div class="h-3 w-3 animate-spin rounded-full border border-gray-300 border-t-blue-500"></div>
        {/if}
      </div>
  ...
  >
    ...
    {:else if isRendering}
      ...
      <div class="flex h-full items-center justify-center">
        <div class="text-center">
          <div class="mx-auto mb-3 h-12 w-12 animate-spin rounded-full border-4 border-gray-300 border-t-blue-500"></div>
