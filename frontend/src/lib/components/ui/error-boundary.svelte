			...
			<div class="min-w-0 flex-1">
				...
				{#if error.message}
					<details class="mt-2">
						...
						<pre
							class="mt-2 text-xs {styles.description} rounded bg-black/5 p-2 font-mono whitespace-pre-wrap dark:bg-white/5">{error.message}</pre>
						{#if error.stack}
							<pre
								class="mt-1 text-xs {styles.description} max-h-32 overflow-y-auto rounded bg-black/5 p-2 font-mono whitespace-pre-wrap dark:bg-white/5">{error.stack}</pre>
						{/if}
					</details>
				{/if}
				...
				{#if showRetry || showDismiss}
					<div class="mt-4 flex gap-2">
						{#if showRetry && onRetry}
							<Button variant="outline" size="sm" onclick={onRetry} class="text-sm">
						...
						{#if showDismiss && onDismiss}
							<Button variant="ghost" size="sm" onclick={onDismiss} class="text-sm">
