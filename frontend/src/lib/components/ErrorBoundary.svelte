...
<script lang="ts">
  ...
  function categorizeError(err: Error): {
    ...
    if (message.includes('memory') || message.includes('allocation') || message.includes('out of memory')) {
      return {
        type: 'memory',
        userFriendlyMessage: 'Not enough memory available. Try closing other browser tabs or using a smaller image.',
        recoverable: true
      };
    }
    ...
    if (message.includes('wasm') || message.includes('webassembly') || message.includes('panic')) {
      return {
        type: 'wasm',
        userFriendlyMessage: 'Processing engine encountered an error. The system will attempt to recover automatically.',
        recoverable: true
      };
    }
    ...
    if (message.includes('validation') || message.includes('invalid') || message.includes('parameter')) {
      return {
        type: 'validation',
        userFriendlyMessage: 'Invalid settings detected. Please check your parameter values and try again.',
        recoverable: true
      };
    }
    ...
    return {
      type: 'unknown',
      userFriendlyMessage: 'An unexpected error occurred. Please try refreshing the page if the problem persists.',
