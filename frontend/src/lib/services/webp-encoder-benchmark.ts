...
export class WebPEncoderBenchmark {
  ...
  private analyzeResults(results: BenchmarkResult['results']): BenchmarkResult['performance'] {
    ...
    for (const [name, result] of Object.entries(results)) {
      if (result && result.successRate > 0.5) { // Only include mostly successful tests
        times.push({ name, time: result.averageTime });
        qualities.push({ name, quality: result.qualityScore });
      }
    }

    const speedRanking = times
      .sort((a, b) => a.time - b.time)
      .map(item => item.name);

    const qualityRanking = qualities
    ...
    if (results.wasm && results.wasm.compressionRatio > (results.canvas?.compressionRatio || 0) * 1.1) {
      recommendations.push('WASM encoder provides significantly better compression');
    }
    
    if (results.webgpu && results.webgpu.averageTime < (results.canvas?.averageTime || Infinity) * 0.8) {
  ...
  private async generateTestImages(testSizes: number[], imageTypes: ImageTestType[]): Promise<void> {
  ...
  private generateLineArtPattern(data: Uint8ClampedArray, width: number, height: number): void {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const isLine = (x % 20 < 2) || (y % 20 < 2) || 
                      (Math.abs(x - y) < 2) || 
                      (Math.abs(x + y - width) < 2);
   ...
   */
  getRecommendedEncoder(imageType: ImageTestType, imageSize: number, systemCapabilities: any): string {
