...
- Software fallback rendering will work reliably as backup
      ...
      const strategies = [
        { powerPreference: 'high-performance' as const, forceFallbackAdapter: false, name: 'High-performance' },
        ...
        { powerPreference: 'high-performance' as const, forceFallbackAdapter: true, name: 'Software-fallback' }
      ];
      ...
      for (let i = 0; i < strategies.length; i++) {
        ...
        try {
          ...
          if (!adapter && strategy.forceFallbackAdapter) {
            ...
            console.warn(`[WebGPU Manager] ⚠️ Software fallback adapter unavailable - this may indicate limited WebGPU support`);
          }
          
          if (adapter) {
            const info = adapter.info || {};
            const gpuClass = this.classifyGPUType(info.device, info.vendor, strategy.forceFallbackAdapter);
            ...
            console.log(`[WebGPU Manager]   Type: ${info.type || (strategy.forceFallbackAdapter ? 'software GPU' : 'unknown')}`);
    ...
    for (const preferredStrategy of preferredStrategies) {
      try {
        ...
        if (freshAdapter) {
          ...
          const gpuClass = this.classifyGPUType(info.device, info.vendor, requestOptions.forceFallbackAdapter);
          const memoryCapacity = this.classifyMemoryCapacity(freshAdapter.limits);
          
          console.log(`[WebGPU Manager] ✅ Got fresh ${preferredStrategy} adapter for device creation`);
          return {
        ...
        console.warn(`[WebGPU Manager] Failed to get fresh ${preferredStrategy} adapter: ${error?.message}`);
      }
    }
  ...
  private async findBestAdapter(powerPreference?: 'low-power' | 'high-performance'): Promise<{
    ...
    console.log(`[WebGPU Manager] Testing ${preferredOrder.length} adapters in order of preference...`);

    for (const adapterInfo of preferredOrder) {
      try {
        console.log(`[WebGPU Manager] Testing ${adapterInfo.strategy} adapter (${adapterInfo.gpuClass} GPU, ${adapterInfo.memoryCapacity} memory)`);
        ...
        if (testResult.success) {
          console.log(`[WebGPU Manager] ✅ Selected ${adapterInfo.strategy} adapter (${adapterInfo.gpuClass} GPU)`);
          return {
            adapter,
          ...
          console.warn(`[WebGPU Manager] ${adapterInfo.strategy} adapter failed device creation test`);
        }
      } catch (error: any) {
        console.warn(`[WebGPU Manager] Error testing ${adapterInfo.strategy} adapter: ${error?.message}`);
      }
    }
   ...
   */
  private async testAdapterDeviceCreation(adapter: any, adapterInfo: any, maxRetries: number = 2): Promise<{ success: boolean }> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      try {
        ...
        deviceLostPromise = testDevice.lost.then((info: any) => {
          deviceLost = true;
          console.warn(`[WebGPU Manager] Test device lost during attempt ${attempt + 1}: ${info.message || 'Unknown reason'}`);
          return info;
        });
        
        // Quick check if device was lost immediately
        if (testDevice.lost && (await Promise.race([deviceLostPromise, Promise.resolve('timeout')]) !== 'timeout')) {
        ...
        for (const testSize of testSizes) {
          try {
            ...
            console.warn(`[WebGPU Manager] Buffer creation failed at ${testSize} bytes: ${bufferError?.message}`);
        ...
        console.log(`[WebGPU Manager] ✅ Device test passed - minimal buffer allocation (${actualBufferSize} bytes) successful on attempt ${attempt + 1}`);
        return { success: true };
        ...
        if (isLastAttempt) {
          console.warn(`[WebGPU Manager] Device test failed after ${maxRetries} attempts: ${errorMessage}`);
        } else {
          console.warn(`[WebGPU Manager] Device test failed on attempt ${attempt + 1}: ${errorMessage}, retrying...`);
    ...
    if (isErrorLoop) {
      console.error('[WebGPU Manager] 🚨 Error loop detected in device testing - further tests disabled');
    }
   ...
   */
  private classifyGPUType(device?: string, vendor?: string, isFallback?: boolean): 'discrete' | 'integrated' | 'software' | 'unknown' {
    if (isFallback) {
      return 'software';
    }
    ...
    if (deviceLower.includes('rtx') || deviceLower.includes('gtx') || 
        deviceLower.includes('radeon') || deviceLower.includes('rx ') ||
        vendorLower.includes('nvidia') || vendorLower.includes('amd')) {
      return 'discrete';
    }
    
    // Check for integrated GPU indicators
    if (deviceLower.includes('intel') || deviceLower.includes('iris') || 
        deviceLower.includes('uhd') || deviceLower.includes('integrated')) {
      return 'integrated';
    }
  ...
  private estimateMemoryCapacity(device?: string, vendor?: string, gpuClass?: string): 'high' | 'medium' | 'low' | 'unknown' {
    const deviceLower = (device || '').toLowerCase();
    
    // High-end discrete GPUs
    if (deviceLower.includes('4090') || deviceLower.includes('4080') || 
        deviceLower.includes('7900') || deviceLower.includes('6900')) {
      return 'high';
    }
    
    // Medium-tier GPUs
    if (deviceLower.includes('4070') || deviceLower.includes('3080') || 
        deviceLower.includes('6800') || deviceLower.includes('5700')) {
      return 'medium';
    }
  ...
  private determineAdapterPreference(adapters: AdapterInfo[], powerPreference?: string): AdapterInfo[] {
  ...
  async getDevice(owner: string, options: any = {}): Promise<any> {
    ...
    console.log(`[WebGPU Manager] ✅ Created device for ${owner} using ${adapterResult.strategy} adapter`);
    return device;
  }
  ...
  async forceReset(): Promise<void> {
    ...
    console.log(`[WebGPU Manager] ✅ Reset complete: cleared ${deviceCount} devices and ${queueLength} queued requests`);
  }
