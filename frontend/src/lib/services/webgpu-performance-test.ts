 ...
 */
export function generateTestSvg(elementCount: number, width: number = 2000, height: number = 2000): string {
  ...
  for (let i = 0; i < elementCount; i++) {
    ...
    switch (elementType) {
      case 0: // Complex path
        ...
        elements.push(`<path d="${path}" stroke="${color}" stroke-width="${1 + Math.random() * 3}" fill="none" opacity="${0.6 + Math.random() * 0.4}"/>`);
        break;
        
      case 1: // Circle with gradient
        const r = 10 + Math.random() * 30;
        elements.push(`<circle cx="${x}" cy="${y}" r="${r}" fill="url(#${gradientId})" opacity="${0.7 + Math.random() * 0.3}"/>`);
        break;
        
      case 2: // Rectangle
        const w = 20 + Math.random() * 80;
        const h = 20 + Math.random() * 80;
        elements.push(`<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${color}" stroke="${color}" stroke-width="2" opacity="${0.6 + Math.random() * 0.4}"/>`);
        break;
        
      case 3: // Ellipse
        const rx = 15 + Math.random() * 40;
        const ry = 15 + Math.random() * 40;
        elements.push(`<ellipse cx="${x}" cy="${y}" rx="${rx}" ry="${ry}" fill="${color}" opacity="${0.7 + Math.random() * 0.3}"/>`);
        break;
      ...
      case 4: // Polygon
        ...
        elements.push(`<polygon points="${points}" fill="${color}" stroke="${color}" stroke-width="1" opacity="${0.6 + Math.random() * 0.4}"/>`);
        break;
        
      case 5: // Line with complex stroke
        const x2 = Math.random() * width;
        const y2 = Math.random() * height;
        elements.push(`<line x1="${x}" y1="${y}" x2="${x2}" y2="${y2}" stroke="${color}" stroke-width="${2 + Math.random() * 4}" stroke-dasharray="${Math.random() * 10},${Math.random() * 10}" opacity="${0.8 + Math.random() * 0.2}"/>`);
        break;
        
      case 6: // Text (occasional)
        if (i % 50 === 0) {
          elements.push(`<text x="${x}" y="${y}" font-family="Arial" font-size="${12 + Math.random() * 16}" fill="${color}" opacity="${0.8 + Math.random() * 0.2}">Test ${Math.floor(i / 50)}</text>`);
        }
        break;
    }
  }
...
export class WebGPUPerformanceTester {
  ...
  private async runSingleTest(elementCount: number): Promise<TestResult> {
    ...
    for (let i = 0; i < this.config.iterations; i++) {
      try {
        ...
        console.log(`     ✅ Iteration ${i + 1}: ${Math.round(endTime - startTime)}ms (${webgpuResult.method})`);
    ...
    if (result.results.webgpu && result.results.baseline) {
      ...
      if (baselineTime > 0) {
        ...
        console.log(`   🎯 Performance: ${speedup.toFixed(2)}x faster (${percentageFaster.toFixed(1)}% improvement)`);
      }
    }
  ...
  private assessVisualQuality(compressionRatio: number): 'excellent' | 'good' | 'acceptable' | 'poor' {
  ...
  private generateRecommendations(testResults: TestResult[]): string[] {
    ...
    const averageImprovement = successfulTests.reduce((sum, t) => sum + (t.results.improvement?.percentageFaster || 0), 0) / successfulTests.length;
    
    if (averageImprovement > 100) {
      recommendations.push('WebGPU provides excellent acceleration (>2x faster) - highly recommended');
    } else if (averageImprovement > 50) {
      recommendations.push('WebGPU provides good acceleration (>1.5x faster) - recommended for large images');
    } else if (averageImprovement > 20) {
      recommendations.push('WebGPU provides moderate acceleration - recommended for complex SVGs');
    } else {
      recommendations.push('WebGPU provides minimal acceleration - consider CPU processing for simple images');
    }

    // Size-based recommendations
    const largeImageTests = successfulTests.filter(t => t.svgSize > 5000000); // >5MB
    if (largeImageTests.length > 0) {
      const largeImageImprovement = largeImageTests.reduce((sum, t) => sum + (t.results.improvement?.percentageFaster || 0), 0) / largeImageTests.length;
  ...
  private printBenchmarkSummary(summary: BenchmarkSummary): void {
    ...
    console.log('\n📊 Detailed Results:');
    console.log('─'.repeat(70));
    console.log('Test Case'.padEnd(20) + 'SVG Size'.padEnd(12) + 'WebGPU'.padEnd(10) + 'Baseline'.padEnd(10) + 'Improvement');
    console.log('─'.repeat(70));

    for (const result of summary.testResults) {
      const svgSizeStr = `${Math.round(result.svgSize / 1024)}KB`;
      const webgpuStr = result.results.webgpu ? `${Math.round(result.results.webgpu.averageTime)}ms` : 'Failed';
      const baselineStr = result.results.baseline ? `${Math.round(result.results.baseline.averageTime)}ms` : 'N/A';
      const improvementStr = result.results.improvement ? `+${result.results.improvement.percentageFaster.toFixed(1)}%` : 'N/A';
