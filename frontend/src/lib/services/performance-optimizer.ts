...
export class PerformanceOptimizer {
	...
	private applyDeviceOptimizations(config: VectorizerConfig) {
		...
		const { isLowEndDevice, estimatedProcessingPower, supportsWasmThreads } =
	...
	private applyImageOptimizations(
		config: VectorizerConfig,
		imageCharacteristics: ImageCharacteristics
	) {
		const { pixelCount, estimatedComplexity } = imageCharacteristics;
	...
	private getMaxProcessingTime(priority: 'speed' | 'quality' | 'balanced'): number {
		switch (priority) {
			case 'speed':
				return 15_000; // 15 seconds max
			case 'quality':
				return 120_000; // 2 minutes max
			case 'balanced':
				return 60_000; // 1 minute max
		}
	}
