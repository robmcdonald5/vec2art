...
export class OptimizedWebPEncoder {
   ...
   */
  async encodeToWebP(imageData: ImageData, options: OptimizedWebPOptions = {}): Promise<WebPEncodingResult> {
  ...
  private async encodeWithWorker(imageData: ImageData, options: { quality: number; adaptive: boolean }): Promise<any> {
    if (!this.worker) {
  ...
  private quickImageAnalysis(imageData: ImageData): ImageCharacteristics {
    ...
    return {
      ...
      dominantColors: Math.min(dominantColors, 256),
      edgeCount,
      textureAmount: edgeCount > sampleCount * 0.15 ? 'heavy' : 
                     edgeCount > sampleCount * 0.08 ? 'moderate' : 'minimal',
      aspectRatio: width / height,
      pixelDensity: width * height
    };
  }
  ...
  private async resizeImageData(imageData: ImageData, maxWidth: number, maxHeight: number): Promise<ImageData> {
    const { width, height } = imageData;
      ...
      const batchPromises = batch.map((imageData, index) => 
        this.encodeToWebP(imageData, {
          ...
          onProgress: (stage, percent) => {
            const overallProgress = ((i + index) / images.length * 100) + (percent / images.length / batchSize);
            options.onProgress?.(`Batch ${Math.floor(i / batchSize) + 1}: ${stage}`, overallProgress);
          }
        })
      );
