...
export class WebGPUImageProcessor {
  ...
  private async createComputePipeline(name: string, shaderCode: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Shader compilation failed for ${name}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async processResize(task: ImageProcessingTask, startTime: number): Promise<ProcessingResult> {
    ...
    if (memoryError || validationError) {
      ...
      throw new Error('Failed to allocate GPU buffers: ' + (memoryError?.message || validationError?.message));
    }
    ...
    commandEncoder.copyBufferToBuffer(outputBuffer, 0, readBuffer, 0, targetWidth * targetHeight * 4);
    ...
    console.log(`[WebGPU] Resize completed: ${sourceWidth}x${sourceHeight} → ${targetWidth}x${targetHeight} in ${Math.round(totalTime)}ms`);
  ...
  private async processEnhance(task: ImageProcessingTask, startTime: number): Promise<ProcessingResult> {
  ...
  private async processCPUFallback(task: ImageProcessingTask, startTime: number): Promise<ProcessingResult> {
  ...
  static async getSystemCapabilities(): Promise<{
    ...
    try {
      ...
      console.warn(`[WebGPU] Coordinated assessment failed: ${error instanceof Error ? error.message : String(error)}`);
      return {
        webgpuSupported: true,
        memoryAllocationLikely: false,
        recommendWebGPU: false,
