...
import { WebGPUImageProcessor, type ImageProcessingTask, type ProcessingResult } from './webgpu-image-processor';
...
export class WebGPUWebPConverter {
  ...
  async convertSvgToWebP(
    ...
    try {
      ...
      const canvasSize = this.calculateOptimalSize(dimensions.width, dimensions.height, maxWidth, maxHeight, scaleFactor);
  ...
  static getRecommendedSettings(svgContent: string): WebGPUWebPOptions {
    ...
    if (size > 10000000) { // >10MB - Very large SVGs
      return {
        quality: 0.75,
        maxWidth: 3840,
        maxHeight: 3840,
    ...
    } else if (size > 5000000) { // >5MB - Large SVGs
      return {
        quality: 0.8,
        maxWidth: 2560,
        maxHeight: 2560,
    ...
    } else if (size > 1000000) { // >1MB - Medium SVGs
      return {
        quality: 0.85,
        maxWidth: 2048,
        maxHeight: 2048,
