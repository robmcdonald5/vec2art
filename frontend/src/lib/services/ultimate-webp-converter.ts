...
import { WebGPUWebPConverter, type WebGPUWebPResult, type WebGPUWebPOptions } from './svg-webp-converter-webgpu';
import { OptimizedWebPEncoder, type OptimizedWebPOptions, type WebPEncodingResult } from './optimized-webp-encoder';
import { ProgressiveWebPStreamer, type ProgressiveStreamOptions, type ProgressiveStage, type StreamingResult } from './progressive-webp-streamer';
...
export class UltimateWebPConverter {
      ...
      if (memoryStatus.canAllocate) {
        ...
        const device = await deviceManager.getDevice('webp-converter', {
          ...
          requiredLimits: {
            ...
            maxStorageBufferBindingSize: Math.min(32 * 1024 * 1024, memoryStatus.recommendedBufferSize)
          }
        });
        
        if (device) {
          console.log('[UltimateWebP] WebGPU available with buffer size:', memoryStatus.recommendedBufferSize);
  ...
  private async initializeCapabilities(): Promise<void> {
    try {
      ...
      this.systemCapabilities.webgpuAvailable = this.webgpuConverter ? WebGPUWebPConverter.isWebGPUSupported() : false;
   ...
   */
  async convertSvgToWebP(svgContent: string, options: UltimateConversionOptions = {}): Promise<UltimateConversionResult> {
    ...
    try {
      ...
      if (conversionMethod !== 'fallback') {
        onPerformanceWarning?.('Primary method failed, using fallback', error instanceof Error ? error.message : String(error));
  ...
  private selectOptimalMethod(params: {
    ...
    const { svgSize, isLargeImage, isComplexImage, preferWebGPU, enableProgressive, enableWasm, wasmLossless, systemCapabilities } = params;
    ...
    if (preferWebGPU && enableWasm && systemCapabilities.webgpuAvailable && 
        systemCapabilities.wasmAvailable && isLargeImage && 
    ...
    if (enableWasm && systemCapabilities.wasmAvailable && 
        (isComplexImage || svgSize < 1000000)) { // WASM good for detailed work
      return 'wasm-optimized';
    }
    
    // WebGPU + Progressive for best experience on capable systems
    if (preferWebGPU && systemCapabilities.webgpuAvailable && 
        enableProgressive && systemCapabilities.progressiveSupported && 
        isLargeImage) {
    ...
    if (enableProgressive && systemCapabilities.progressiveSupported && 
  ...
  // Utility methods
  private async createImageDataFromSvg(svgContent: string, options: UltimateConversionOptions): Promise<ImageData> {
    // Create SVG image
    const img = new Image();
  ...
  private analyzeImageForWasm(imageData: ImageData): 'line-art' | 'photo' | 'mixed' {
    ...
    for (let i = 0; i < data.length; i += 16 * sampleStep) { // RGBA = 4 bytes, step by 4 pixels
      const r = data[i];
      const g = data[i + 1];
      const b = data[i + 2];
      const a = data[i + 3];
