...
class VectorizerWorker {
  ...
  private checkCapabilities(): WasmCapabilityReport {
    ...
    try {
      ...
      const threading_supported = typeof this.wasmModule.is_threading_supported === 'function' 
      ...
      return {
        ...
        cross_origin_isolated: typeof self !== 'undefined' && 'crossOriginIsolated' in self ? self.crossOriginIsolated : false,
  ...
  private countSvgDots(svg: string): number {
    ...
    return (circleMatches ? circleMatches.length : 0) + (ellipseMatches ? ellipseMatches.length : 0);
  }
