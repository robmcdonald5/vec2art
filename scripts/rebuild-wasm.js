...
async function cleanBuild() {
  ...
  // Custom files to preserve during clean (not generated by wasm-pack)
  const preserveFiles = new Set([
    'worker-load.js',
    'worker-load.d.ts'
  ]);

  // Clean WASM files from frontend (except preserved custom files)
  try {
    const files = await fs.readdir(config.frontendWasmDir);
    for (const file of files) {
      if (preserveFiles.has(file)) {
        continue; // Don't delete custom worker files
      }
 ...
 * This file provides stub implementations for wasm-bindgen intrinsic functions
 * that are imported by the WASM binary from the __wbindgen_placeholder__ module.
 * These functions are compile-time descriptors processed by wasm-bindgen CLI.
 */
...
async function copyFiles() {
  ...
  // Preserve custom worker files from static directory before overwrite
  // These are not generated by wasm-pack but are needed for web workers
  const customFilesToPreserve = [
    'worker-load.js',
    'worker-load.d.ts'
  ];

  for (const file of customFilesToPreserve) {
    const staticSrc = path.join(config.staticWasmDir, file);
    const dest = path.join(config.frontendWasmDir, file);
    try {
      await fs.access(staticSrc);
      await fs.copyFile(staticSrc, dest);
      log.info(`Preserved custom file: ${file}`);
    } catch {
      // File doesn't exist in static, skip
    }
  }

  // Files to copy from wasm-pack output
  const files = [
    'vectorize_wasm.js',
    'vectorize_wasm.d.ts',
  ...
  // Update pkg-simd subdirectory (used by SIMD loader in loader.ts)
  // Note: The loader uses pkg-simd/ for SIMD-capable browsers and main directory for standard
  // The pkg/ directory is historical and not actively used by the loader
  const pkgSimdDir = path.join(config.frontendWasmDir, 'pkg-simd');
  const filesToCopyToPkg = [
    'vectorize_wasm.js',
    'vectorize_wasm.d.ts',
    'vectorize_wasm_bg.wasm',
    'vectorize_wasm_bg.wasm.d.ts',
  ...
  try {
    await fs.access(pkgSimdDir);
    for (const file of filesToCopyToPkg) {
      const src = path.join(config.frontendWasmDir, file);
    ...
    log.info('Updated pkg-simd/ subdirectory');
  } catch {
    // Subdirectory doesn't exist, skip
  }

  // Also update pkg/ for backwards compatibility (may be removed in future)
  const pkgDir = path.join(config.frontendWasmDir, 'pkg');
  try {
    ...
    log.info('Updated pkg/ subdirectory (legacy)');
  } catch {
    // Subdirectory doesn't exist, skip
  }
